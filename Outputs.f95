subroutine Initialize_Outputs
  ...
  !**** This file may be used by a browsing program to plot rangeland maps.
  !****
  !**** R.B. Boone       Last modified:  May 17, 2023
  use Structures
  use Parameter_Vars
  implicit none
...
!      '    // Range cells, XDim, YDim, LX, LY, HX, HY, Cell size, First yr, Last Yr. Below: Cell ID, Zone, X, Y, Landscape, Cover'
    do icell = 1, range_cells
      ix = Rng(icell)%x
      iy = Rng(icell)%y
      write(SHORT_USE_FILE,1111) icell,c, Area(ix,iy)%zone,c, Rng(icell)%x,c, Rng(icell)%y,c, Rng(icell)%range_type
!       &  ,c, Area(ix,iy)%class
    end do
...
1001  format(5000L1)                                    ! This value is hardwired
  ...
  do ix = 1, WHOLE_MAP_COUNT
    ...
    if (Outs(ix)%write_out .eqv. .TRUE.) then
...
subroutine Output_Surfaces
  ...
  !
  !**** R.B. Boone       Last modified:  January 20, 2025 ... added establishment and death coefficient surfaces
  use Structures
  use Parameter_Vars
  implicit none
  ...
  do i = 1, WHOLE_MAP_COUNT
    if ( 'x_loc' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'y_loc' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'range_type' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  ! These are the only variables stored in Area that are output.
  ! Precipitation
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'precip' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
      rng_out(icell) = Area(Rng(icell)%x, Rng(icell)%y)%precip
    end do
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'max_temp' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
      rng_out(icell) = Area(Rng(icell)%x, Rng(icell)%y)%max_temp
    end do
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'min_temp' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
      rng_out(icell) = Area(Rng(icell)%x, Rng(icell)%y)%min_temp
    end do
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'day_length' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'heat_accumulation' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'pot_evap' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'evaporation' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'snow' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'snow_liquid' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'melt' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'pet_remaining' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'ppt_soil' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'runoff' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'ratio_water_pet' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
...
!  write_it = .FALSE.
!  do i = 1, WHOLE_MAP_COUNT
!    if ( 'co2_value' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
!  end do
!  if ( write_it .eqv. .TRUE. ) then
!    do icell = 1, range_cells
!      rng_out(icell) = Rng(icell)%co2_value
!    end do
!    call Output_One_Surface ('co2_value                        ', 9, rng_out)
!  end if

  ! Potential evapotranspiration from top soil
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'pet_top_soil' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'n_leached' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'holding_tank' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'transpiration' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'relative_water_content' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, SOIL_LAYERS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'water_available' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, 3
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'annual_evapotranspiration' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'facet_cover' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'bare_cover' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'total_population' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, V_LYRS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'prop_annual_decid' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'total_aground_live_biomass' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'total_bground_live_biomass' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'total_litter_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, 2
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'total_litter_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, 2
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'root_shoot_ratio' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'tree_basal_area' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'soil_surface_temperature' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'sand' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, SOIL_LAYERS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'silt' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, SOIL_LAYERS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'clay' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, SOIL_LAYERS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'mineral_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, SOIL_LAYERS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'field_capacity' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, SOIL_LAYERS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'wilting_point' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, SOIL_LAYERS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'soil_total_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'tree_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'tree_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ipart = 1, WOODY_PARTS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'shrub_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ipart = 1, WOODY_PARTS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'shrub_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ipart = 1, WOODY_PARTS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'carbon_nitrogen_ratio' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, 2
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'fast_soil_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, 2
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'intermediate_soil_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'passive_soil_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'fast_soil_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, 2
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'intermediate_soil_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'passive_soil_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'potential_production' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'belowground_pot_production' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, V_LYRS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'aboveground_pot_production' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, V_LYRS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'total_pot_production' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, V_LYRS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'co2_effect_on_production' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'total_pot_prod_limited_by_n' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, V_LYRS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'monthly_net_primary_prod' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'fraction_live_removed_grazing' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'fraction_dead_removed_grazing' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  ! Herbivore_populations          ! NOTE NOTE: Up to 10 animals are supported.  BUT here the seven included in Af_Range are going to be produced.
  !                                !            If additional animal species are included, the browsing tool or the like will require edit (this will not)
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'herbivore_populations' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, herbivores
      do icell = 1, range_cells
        rng_out(icell) = Rng(icell)%herbivore(ilayer)
      end do
      call Output_One_Surface ('herbivore_populations            ', 21, rng_out)
    end do
  end if

  ! Herbivore_biomasses            ! NOTE NOTE: Up to 10 animals are supported.  BUT here the seven included in Af_Range are going to be produced.
  !                                !            If additional animal species are included, the browsing tool or the like will require edit (this will not)
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'herbivore_biomasses' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, herbivores
      do icell = 1, range_cells
        rng_out(icell) = Rng(icell)%herbivore_biomass(ilayer)
      end do
      call Output_One_Surface ('herbivore_biomasses              ', 19, rng_out)
    end do
  end if
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'temp_effect_on_decomp' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'water_effect_on_decomp' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'anerobic_effect_on_decomp' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'all_effects_on_decomp' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_fine_root_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_fine_root_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_standing_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_standing_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_seed_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_seed_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_leaf_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_leaf_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_fine_branch_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_total_fine_branch_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_fine_branch_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_total_fine_branch_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_coarse_root_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_total_coarse_root_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_coarse_root_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_total_coarse_root_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_coarse_branch_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_total_coarse_branch_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_coarse_branch_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'dead_total_coarse_branch_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'lignin_fine_root' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'lignin_coarse_root' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'lignin_fine_branch' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'lignin_coarse_branch' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'lignin_leaf' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
...
!  write_it = .FALSE.
!  do i = 1, WHOLE_MAP_COUNT
!    if ( 'plant_lignin_fraction' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
!  end do
!  if ( write_it .eqv. .TRUE. ) then
!    do ifacet = 1, FACETS
!      do ilayer = 1, 2
...
!  end if
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'litter_structural_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, 2
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'litter_metabolic_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, 2
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'litter_structural_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, 2
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'litter_metabolic_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ilayer = 1, 2
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'maintain_respiration' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'phenology' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'fine_root_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'fine_root_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'seed_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'seed_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'leaf_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'leaf_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'fine_branch_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'fine_branch_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'coarse_root_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'coarse_root_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'coarse_branch_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'coarse_branch_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'stored_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'plant_nitrogen_fixed' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'nitrogen_fixed' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'respiration_flows' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'respiration_annual' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'optimum_leaf_area_index' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'leaf_area_index' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'water_function' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'carbon_source_sink' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'nitrogen_source_sink' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'fire_severity' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'burned_carbon' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'burned_nitrogen' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'fertilized_nitrogen_added' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'fertilized_carbon_added' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  ! Water effect on establishment (FACETS)
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'water_effect_on_establish' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
        rng_out(icell) = Rng(icell)%water_effect_on_establish(ifacet)
      end do
      call Output_One_Surface ('water_effect_on_establish        ', 25, rng_out)
    end do
  end if

  ! Herb root effect on establishment (FACETS)
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'herb_root_effect_on_establish' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
        rng_out(icell) = Rng(icell)%herb_root_effect_on_establish(ifacet)
      end do
      call Output_One_Surface ('herb_root_effect_on_establish        ', 29, rng_out)
    end do
  end if

  ! Litter effect on establishment (FACETS)
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'litter_effect_on_establish' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
        rng_out(icell) = Rng(icell)%litter_effect_on_establish(ifacet)
      end do
      call Output_One_Surface ('litter_effect_on_establish        ', 26, rng_out)
    end do
  end if

  ! Woody cover effect on establishment (FACETS)
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'woody_cover_effect_on_establish' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
        rng_out(icell) = Rng(icell)%woody_cover_effect_on_establish(ifacet)
      end do
      call Output_One_Surface ('woody_cover_effect_on_establish        ', 31, rng_out)
    end do
  end if

  ! Water effect on death rate
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'water_effect_on_death_rate' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
        rng_out(icell) = Rng(icell)%water_effect_on_death_rate(ifacet)
      end do
      call Output_One_Surface ('water_effect_on_death_rate             ', 26, rng_out)
    end do
  end if

  ! Grazing effect on death rate
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'grazing_effect_on_death_rate' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
        rng_out(icell) = Rng(icell)%grazing_effect_on_death_rate(ifacet)
      end do
      call Output_One_Surface ('grazing_effect_on_death_rate             ', 28, rng_out)
    end do
  end if

  ! Shading effect on death rate
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'shading_effect_on_death_rate' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do ifacet = 1, FACETS
      do icell = 1, range_cells
        rng_out(icell) = Rng(icell)%shading_effect_on_death_rate(ifacet)
      end do
      call Output_One_Surface ('shading_effect_on_death_rate             ', 28, rng_out)
    end do
  end if
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'large_error_count' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
  ...
  write_it = .FALSE.
  do i = 1, WHOLE_MAP_COUNT
    if ( 'neg_error_count' .eq. Outs(i)%whole_map_output .and. Outs(i)%write_out .eqv. .TRUE. ) write_it = .TRUE.
  end do
  if ( write_it .eqv. .TRUE. ) then
    do icell = 1, range_cells
...
subroutine Output_One_Surface (file_root, n, rng_out)
  ...
  !****
  !**** rng_out stores a the contents to be written out.
  !****
  ...
  open(SHORT_USE_FILE, FILE=out_file, ACTION='WRITE', POSITION='APPEND', FORM='UNFORMATTED', ACCESS='STREAM', IOSTAT=ioerr)
  if (ioerr == 0) then
