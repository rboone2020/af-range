...
!****
!**** R. Boone   Last modified: May 18, 2023
!****                           Modified how average biomass was calculated.  It was calculating only a facet at a time, instead of the average of all the cell
!****                           Modifying the way aboveground live biomass is calculated, but also moving it to Update_Vegetation.  Here it was being updated only when snow free.
!****                           Modified in 2023 due to a catch by R. Warrier noting that transpiration and CO2 fertilization is inversely related to production CO2 fertilization
  ...
  real     fwloss_1, fwloss_2, avg_co2eff
  integer  icell, soil_layer, iunit

  iunit = Rng(icell)%range_type
  fwloss_1 = 1.0                           ! A variable in CENTURY, but defaults to 1.0
  fwloss_2 = 1.9                           ! A variable in CENTURY, but defaults to 1.0.  Include as needed.  Estimates low, so adjusted upward.
  ...
  if (Rng(icell)%melt .gt. 0.0) then
    ...
    Rng(icell)%ratio_water_pet = ( Rng(icell)%water_available(3) + Area(Rng(icell)%x, Rng(icell)%y)%precip ) / &
  ...
  if (temp_avg .lt. 2.0) then
    ...
    ! Modified 05/2023.  CO2 effect on transpiration made the inverse of the CO2 effect on production as per "Century_curves.xlsx"
    avg_co2eff = ( Rng(icell)%co2_effect_on_production(H_FACET) +  Rng(icell)%co2_effect_on_production(S_FACET) + &
                 Rng(icell)%co2_effect_on_production(T_FACET) ) / 3       ! NOT facet-based.  Water loss be based on the average
    pttr = Rng(icell)%pet_remaining * 0.65 * (1.0 - exp(-0.020 * avg_live_biomass)) * (1.0 + (1.0 - avg_co2eff))
  end if
  ...
  if (tot2 .gt. 0.0) then
    do soil_layer=1,SOIL_LAYERS
      ...
      avinj = avinj - trl
  !write(*,*) 'ZZZ TRL: ', icell, Rng(icell)%transpiration, trl
  ...
  !write(*,*) 'ZZZ AE EA=A+T: ', icell, Rng(icell)%annual_evapotranspiration, Rng(icell)%evaporation, Rng(icell)%transpiration
  ...
  if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'H2O_LSS')
