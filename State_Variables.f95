...
!****
!**** R. Boone   Last modified: December 28, 2013  ... updated to incorporate all the changes made to variables
   use Parameter_Vars
   use Structures
   implicit none
   ...
   if (ioerr == 0) then
     ...
     do icell = 1, range_cells
       write(SHORT_USE_FILE,*) 'zone       : ', Area(Rng(icell)%x, Rng(icell)%y)%zone
...
!****
!**** R. Boone   Last modified: Feb 10, 2025   Edited to allow state variables to be used but not overwrite facets (Sim_Parm%state_var_flag == 4)
   use Parameter_Vars
   use Structures
   implicit none

   character(255)  str
   integer  i, j, icell, xtemp, ytemp, celltemp, rangetemp
   real lxtemp, lytemp, uxtemp, uytemp
   integer  zonetemp
   real     cellsizetemp, dummy, dummy3(FACETS), dummy6(V_LYRS)
   ...
   if (ioerr == 0) then
     ...
     do icell = 1, range_cells
       ...
       str = trim(adjustl(str(15:200)))
       read(str,'(i10)') zonetemp
       if (zonetemp .ne. Area(Rng(icell)%x, Rng(icell)%y)%zone) then
         write(*,*) 'The zonal id does not match what was read in from the map.'
         write(*,*) 'The value in the simulation is: ',Area(Rng(icell)%x, Rng(icell)%y)%zone
       ...
       if (Sim_Parm%state_var_flag .ne. 4) then
         ...
         read(SHORT_USE_FILE,*) (dummy3(i),i=1,FACETS)             ! Discard values
         read(SHORT_USE_FILE,*) (dummy6(i),i=1,V_LYRS)             ! Discard values
         read(SHORT_USE_FILE,*) dummy                              ! Discard values
       end if
