module Structures
  ...
  ! R. Boone    Last modified:  March 18, 2023.  Edited to include herbivores
  !                             January 20, 2025.  Edited to include establishment and death rate coefficient storage
  !                             March 19, 2025.  Edited to include herbivore densities
  !                             March 5, 2026.  Edited to include specific-specific density multipliers to observed
  use Parameter_Vars

  ! The following stores attributes for all parts of the area, including non-range and even ocean. That is most
  ! straightforward programatically, and memory is inexpensive now.  So here a trade-off is made in memory use
  ! versus clarity of programming.  But if the memory requirements become excessive, a redesign would put these
  ! parameters in a new structure storing only rangeland cells. (But the current representation has a big
  ! advantage in that different areas may be called rangelands by different users and the system would still
  ! have all the base data.)

  !********************************************************
  ! Stores attributes of cells comprising the area.
  ...
  type AreaCell
    ...
    ! integer    :: elev                    ! Elevation, in meters
    logical    :: area                    ! Defining the study area versus what is simulated as rangeland
    ...
    real       :: herbivore_pop(MAX_HERBIVORES)  ! The initial herbivore populations
    real       :: prop_forage_avail       ! The proportion of forage produced that is available MONTHLY to herbivores
    real       :: temporary               ! A temporary map location, used to read-in values
  end type
  type (AreaCell) :: Area(MAX_X_DIM, MAX_Y_DIM)
...
! Below, in Savanna    real       :: litter_structural_carbon(2)     ! Litter structural carbon, surface and soil   g/m2  (STRCIS in Century)
! Below, in Savanna    real       :: litter_metabolic_carbon(2)      ! Metabolic litter carbon, surface and soil    g/m2  (METCRIS in Century)
    ...
    ! Second effort at decomposition, drawing on Savanna, which excludes phosphorous.
    ...
    ! Addition for modeling herbivores
    integer    :: herbivore(MAX_HERBIVORES)             ! The number of herbivore species represented in the model
    real       :: herbivore_biomass(MAX_HERBIVORES)     ! The mean biomass of ALL the animals of the species in the cell
    real       :: prop_forage_avail                     ! The proportion of forage available for herbivores, which will be cell-specific to capture the many potential differences between food-based capacity and numbers of animals actually on the landscape (e.g., access)

    real       :: fraction_live_grazed(FACETS)          ! Fraction of live biomass removed by herbivores
    real       :: fraction_dead_grazed(FACETS)          ! Fraction of dead biomass removed by herbivores

    real       :: tgreen_herb_biomass                   ! A variable that stores AVAILABLE biomass that is decremented
    real       :: tdead_herb_biomass                    !
    real       :: tgreen_shrub_biomass                  !
    real       :: tdead_shrub_biomass                   !
    real       :: tfbranch_shrub_biomass                !
    real       :: tgreen_tree_biomass                   !
    real       :: tdead_tree_biomass                    !
    real       :: tfbranch_tree_biomass                 !

    real       :: for_writing                           ! A temporary variable storing biomasses prior to writing
    real       :: grazed_green_herb_fraction            ! The proportion of green herbaceous biomass that was consumed by livestock for the cell
    real       :: grazed_dead_herb_fraction             !
    real       :: grazed_green_shrub_fraction           !
    real       :: grazed_dead_shrub_fraction            !
    real       :: grazed_fbranch_shrub_fraction         !
    real       :: grazed_green_tree_fraction            !
    real       :: grazed_dead_tree_fraction             !
    real       :: grazed_fbranch_tree_fraction          !
    real       :: fme(MAX_HERBIVORES)                   ! Fermentalizable metabilizable energy (MJ)
    real       :: energy_used(MAX_HERBIVORES)           ! MJ energy used in the time step

    ! Adding parameters to store plant establishment and death rate coefficients
    real       :: water_effect_on_establish(FACETS)       ! Coefficients controlling the establishment and death rate of plants.
    real       :: herb_root_effect_on_establish(FACETS)   ! These are stored for output, so that maps may be made and viewed to adjust
    real       :: litter_effect_on_establish(FACETS)      ! parameters.
    real       :: woody_cover_effect_on_establish(FACETS) !
    real       :: water_effect_on_death_rate(FACETS)      !
    real       :: grazing_effect_on_death_rate(FACETS)    !
    real       :: shading_effect_on_death_rate(FACETS)    !
  ...
  type SimParm
    ...
    character(100)      :: area_map                                   ! Storing the study area map
    ...
    character(100)      :: herbivore_file_name                        ! The name of the file storing parameters for herbivore parameters
  ...
  !********************************************************
  ! Stores the contents of the herbivore parameter file (e.g., HERBIVORE.GRG) parameter file
  type Herbivore_Parm
    integer             :: herbivores                                 ! Number of herbivore species
    character(100)      :: prop_forage_avail_map                      ! A map showing the proportion of forage available for a year (proportion ... typically very low, given that ca. 10% of any vegetation is consumed and the value is divided by 12 months)
    integer             :: cell_resolution                            ! Cell resolution (in meters; the cell resolution in Ver. 1.13 L-Range is in degrees, here meters are needed)
    character(20)       :: spp_name(MAX_HERBIVORES)                   ! Species names
    character(100)      :: spp_map(MAX_HERBIVORES)                    ! Species file pathway to initial population map
    integer             :: model_dynamics(MAX_HERBIVORES)             ! Whether or not each herbivore species should have dynamics modeled (1) or should be held constant (0)
    real                :: max_density_multiplier(MAX_HERBIVORES)     ! The maximum density of herbivores allowed relative to the observed densities for landscape untis (the same value for all species at this point)
    real                :: min_max_mass_ratio(MAX_HERBIVORES, 2)      ! Ratio of Minimum Body Weight to Mean Body Weight;Ratio of Maximum Body Weight to Mean Body Weight
    real                :: expected_kg(MAX_HERBIVORES)                ! Maximum lean body mass of adult of the species in question
    real                :: min_max_mass_change(MAX_HERBIVORES, 2)     ! Maximum Rate of Weight Loss(kg/day);Maximum Rate of Weight Gain(kg/day)
    real                :: min_max_energy_use(MAX_HERBIVORES, 2)      ! Minimum and maximum energy use rate(megajoules/kg/day)
    real                :: max_energy_use_vs_ci(MAX_HERBIVORES, 6)    ! Fraction of maximum voluntary energy use vs. condition index (the degree to which an animal can adapt to its poor condition by using less energy)
    real                :: daily_offtake_prop(MAX_HERBIVORES)         ! The daily offtake as a proportion of total body mass, often about 2% or 0.02
    real                :: metabilizability(MAX_HERBIVORES)           ! The metabolizability of forage consumed (e.g., higher for ruminants and lower for hind-gut digesters)
    real                :: max_intake_rate(MAX_HERBIVORES, 8)         ! The maximum intake rate (kg/kg/day) for the biomass pools (POOLS ARE: 1) GREEN_HERB, 2) DEAD_HERB, 3) GREEN_SHRUB, 4) DEAD_SHRUB, 5) FINE_BRANCH_SHRUB, 6) GREEN_TREE, 7) DEAD_TREE, 8) TREE_BRANCH)
    real                :: pool_preference(MAX_HERBIVORES, 8)         ! The preference an animal has for the different pools (POOLS ARE: 1) GREEN_HERB, 2) DEAD_HERB, 3) GREEN_SHRUB, 4) DEAD_SHRUB, 5) FINE_BRANCH_SHRUB, 6) GREEN_TREE, 7) DEAD_TREE, 8) TREE_BRANCH)
    real                :: digestibility_fraction(MAX_HERBIVORES, 8)  ! The digestibility fraction of the different biomass pools (POOLS ARE: 1) GREEN_HERB, 2) DEAD_HERB, 3) GREEN_SHRUB, 4) DEAD_SHRUB, 5) FINE_BRANCH_SHRUB, 6) GREEN_TREE, 7) DEAD_TREE, 8) TREE_BRANCH)
    real                :: ungrazable_biomass(MAX_HERBIVORES, 2)      ! The biomass that cannot be grazed - animals cannot eat materials all the way to the ground, and this contributes to sustainability as well.
    real                :: wasted_prop(MAX_HERBIVORES)                ! The proportion of forage wasted by animals, used for all pools.
    real                :: critical_temps(MAX_HERBIVORES, 3)          ! Active and bedded critical temperatures for thermal maintenance, plus proportion of time bedded.
    real                :: thermal_cost_prop(MAX_HERBIVORES)          ! Thermal reg. cost as fraction of maintenance cost per degree C
    real                :: birth_rate_vs_ci(MAX_HERBIVORES, 8)        ! Birth rate versus condition index (i.e., birth rate versus condition index, 4 sets)
    real                :: death_rate_vs_ci(MAX_HERBIVORES, 8)        ! Death rate per year due to poor condition, (i.e., death rate versus condition index, 4 sets)
  end type
  type(Herbivore_Parm)  :: Herbivore_Parms
  ...
  type UnitParm
    ...
    real                :: temperature_production(FACETS,4)           ! Parameters describing the effect of temperature on potential production.  See the Century Parameterization Workbook for examples.
...
!    real                 :: precip_n_symbiotic(2)                      ! Parameters relating precipitation to symbiotic N fixation rate				
    ...
    real                 :: herbivore_density(MAX_HERBIVORES)          ! The observed density per 1 km2 of herbivores
  ...
  type ExceedCnts
    ...
    integer       :: herbivore_population(2)
    integer       :: herbivore_biomass(2)
