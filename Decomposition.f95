...
!****
!**** R. Boone   Last modified: August 8, 2017
!****                           The following line:  co2los = tcdec * p1co2 had a + where the * belonged.
!****                           Also double-checked against SAV5LB, which Coughenour says fixed some errors versus the SAV4L version originally used.
!****                           Added trimming feature for frac_lignin.
!****                           Corrected nitrogen fixation and mineral nitrogen calculation.
  use Parameter_Vars
  use Structures
  implicit none
  ...
  do ifacet = 1, FACETS
    ...
    ! Fire is not transfered here, following Savanna
    if (prf(ifacet) .gt. 0.) then
      ! do itype=1,3    (not used presently)
      ! In the following clauses, I am removing the 0.4 multiplier, which I believe only converted biomass to carbon in Savanna.   Already carbon in G-Range
      ! *** I am also disabling the multiplier by facet area.  I can't imagine why that would be used, except in the nested facet treatment that incorporates mixing used in SAVANNA.
      ! Fine roots
  ...
  do ilayer=SURFACE_INDEX, SOIL_INDEX               ! Confirmed two layers
    ...
    call Track_Lignin(icell, SURFACE_INDEX, 2, Rng(icell)%dead_total_coarse_branch_carbon, &
                     Rng(icell)%dead_total_coarse_branch_nitrogen, Rng(icell)%all_effects_on_decomp, decodt, 0.25, grmin, immobil)
    Rng(icell)%grossmin(ilayer) = Rng(icell)%grossmin(ilayer) + grmin
    Rng(icell)%tnetmin(ilayer) = Rng(icell)%tnetmin(ilayer) - immobil
    if (Rng(icell)%tnetmin(ilayer) .lt. 0.0) Rng(icell)%tnetmin(ilayer) = 0.0
    Rng(icell)%tminup(ilayer) = Rng(icell)%tminup(ilayer) + immobil
  ...
  ! Going a little off the Savanna path here, given that some state variables will exist
  ! Not incorporating the view from Century here.   This is in line with the representation in SAVANNA.
  ...
  fixa = Parms(iunit)%precip_n_deposition(1) * ( 12. / 365. ) + ( Parms(iunit)%precip_n_deposition(2) * &
     Area(Rng(icell)%x, Rng(icell)%y)%precip )
  fixa = max(0., fixa)
  ...
  fxbiom = 1. - biof
  fxbiom = min(1., fxbiom)
  temper = Area(Rng(icell)%x, Rng(icell)%y)%temperature_average
  ...
  ! Inputs due to landscape (water) redistribution, associated with soil type - SKIPPED for now, and in my Savanna applications  (see soilninp.prm)
  ...
  if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'DECOMPN')
...
!**** R. Boone   Last modified: April 9, 2011
  ...
  if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'PARTLIT')
...
!**** These effects are from CENTURY, used in the decomposition from Savanna but programmed before that switch.  But they seem to be reasonable.
!**** Beyond that, there is a great deal of borrowing back-and-forth between the two models.
!****
...
!****            Replacing runoff estimate, which is not well tracked, with drain estimate, which is a variable in Century and now in G-Range.  That variable is a general coefficient, DRAIN=1 for sandy soils, DRAIN=0 for clay soils.
  ...
  ! NOTE: This doesn't mesh exactly with Savanna.  Savanna uses three layers, rather than a surface and soil.
  ...
  if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'EFF_LIG')
...
!****
!**** R. Boone   Last modified: June 19, 2023.  Altered to make woodc compare to 0.0001 instead of 0.0.
!****                           Also, set smintosom to max(0,smintosom) as is done in several parts of the code.
!****                           This is done at the end of the process, in case the if-then is bypassed.
!****                           Otherwise a -NaN can result.
  use Parameter_Vars
  use Structures
  implicit none
  ...
  smintosom = max(0., smintosom)
  if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'TRK_LIG')

end subroutine


subroutine Nitrogen_Losses (icell)
!**** Nitrogen volatization and leaching routine, building on Century's version.  The Savanna version is in DECOMP.F, so this is here as well,
!**** although it may be somewhat misplaced. Century's version was simplified to only deal with nitrogen.
!****
!**** R. Boone   Last modified: October 5, 2013
  ...
  ! if (Rng(icell)%facet_cover .lt. 0.005) return      Removed... Facet not available, and not useful to include here just for this truncation.                     ! If the facet covers almost nothing, do not continue
  ...
  if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'NIT_LOSS')
