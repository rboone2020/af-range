...
!****
!**** R. Boone    Last modified: August 28, 2018
!****             Edited to use future map productions if available, rather than always wrapping at 2000.
!****             Edited to not assume there are 100 years of data, which is not the case for TerraClimate.
!****             Edited to separate the year and month with an underscore.
   use Parameter_Vars
   use Structures
   implicit none
   integer ix, iy, w_year
   character(110) big_string
   character(7) mid_string
   logical file_exists

   ! Create a ** DRAFT ** date string that is a substring of the file names.
   w_year = year
   select case (month)
     case (1) ; write(mid_string, fmt='(I4,A3)') w_year, '_01'
     case (2) ; write(mid_string, fmt='(I4,A3)') w_year, '_02'
     case (3) ; write(mid_string, fmt='(I4,A3)') w_year, '_03'
     case (4) ; write(mid_string, fmt='(I4,A3)') w_year, '_04'
     case (5) ; write(mid_string, fmt='(I4,A3)') w_year, '_05'
     case (6) ; write(mid_string, fmt='(I4,A3)') w_year, '_06'
     case (7) ; write(mid_string, fmt='(I4,A3)') w_year, '_07'
     case (8) ; write(mid_string, fmt='(I4,A3)') w_year, '_08'
     case (9) ; write(mid_string, fmt='(I4,A3)') w_year, '_09'
     case (10) ; write(mid_string, fmt='(I4,A3)') w_year, '_10'
     case (11) ; write(mid_string, fmt='(I4,A3)') w_year, '_11'
     case (12) ; write(mid_string, fmt='(I4,A3)') w_year, '_12'
   end select
   ...
   write(big_string,'(a85,a7,a8)') trim(Sim_Parm%precip_path_prefix),mid_string,adjustl(Sim_Parm%precip_temp_suffix)
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary .lt. 0) then
         Area(ix,iy)%precip = 0                                   ! Will assign null data 0 precip, but that is fine.
       else
         Area(ix,iy)%precip = Area(ix,iy)%temporary / 10.0        ! Stored as mm in the GRIDS, so / 10 takes that to cm.
       end if
     end do
   end do
   ...
   write(big_string,'(a85,a7,a8)') trim(Sim_Parm%max_temp_path_prefix),mid_string,adjustl(Sim_Parm%precip_temp_suffix)
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary .lt. -400.0) then
         Area(ix,iy)%max_temp = -40.
       else
         Area(ix,iy)%max_temp = Area(ix,iy)%temporary               ! SPP TEMPERATURE SURFACES NOT STORED * 10 ...         / 10.0
       end if
     end do
   end do
   ! These maps are not echoed, due to their large size.  Specific maps may be printed as output, rather than echoed, as a check of their form.
   ...
   write(big_string,'(a85,a7,a8)') trim(Sim_Parm%min_temp_path_prefix),mid_string,adjustl(Sim_Parm%precip_temp_suffix)
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary .lt. -400.0) then
         Area(ix,iy)%min_temp = -40.
       else
         Area(ix,iy)%min_temp = Area(ix,iy)%temporary              ! SPP TEMPERATURE SURFACES NOT STORED * 10 ...         / 10.0
       end if
     end do
   end do
   ! These maps are not echoed, due to their large size.  Specific maps may be printed as output, rather than echoed, as a check of their form.
...
!**** R. Boone    Last modified: March 5, 2011
  ...
  Rng(icell)%storm_flow = Parms(iunit)%prcp_threshold_fraction * Area(Rng(icell)%x, Rng(icell)%y)%precip
  temp_mean  = ( Area(Rng(icell)%x,Rng(icell)%y)%max_temp + Area(Rng(icell)%x,Rng(icell)%y)%min_temp ) / 2.0
  ...
  if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'WEATHER')
...
!**** This section draws heavily on the CENTURY code, as much of this material does.  Specially SNOWCENT
!**** R. Boone    Last modified: March 14, 2011
  ...
  sublimated = 0.0
  snow_total = 0.0
  Rng(icell)%ppt_soil = Area(Rng(icell)%x, Rng(icell)%y)%precip
  temp_avg = ( Area(Rng(icell)%x, Rng(icell)%y)%max_temp + Area(Rng(icell)%x, Rng(icell)%y)%min_temp ) / 2.0      ! The method used in Century

  ! Judge whether precipitation is snow or liquid
  if (temp_avg .le. 0.0) then
    Rng(icell)%snow = Rng(icell)%snow + Area(Rng(icell)%x, Rng(icell)%y)%precip           ! Recall snowpack is water equivalent
    accum_snow = Area(Rng(icell)%x, Rng(icell)%y)%precip                                  ! Track snow accumulation
...
!**** R. Boone, almost directly from CENTURY.    Last modified: September 25, 2010
  ...
  radians_lat = Area(Rng(icell)%x, Rng(icell)%y)%latitude * ( PI / 180.0 )
...
!**** R. Boone, drawing from Century 4.5 PEvap.f    Last modified: March 14, 2011
   ...
   const2 = 17.8
   langley2watts = 54.0
   fwloss_4 = 1.48              ! A variable in Century, but defaults to 0.75.  Increasing because of tests to observed to 1.48. 2.0 is cited as the maximum in Necpalova et al. (2015)

   ! Calculate PET for the Julian day in the middle of the current month
   nx = Rng(icell)%x
   ny = Rng(icell)%y
   ! site_latitude = Area(nx,ny)%latitude
   temp_range = Area(nx,ny)%max_temp - Area(nx,ny)%min_temp
   temp_mean  = ( Area(nx,ny)%max_temp + Area(nx,ny)%min_temp ) / 2.0
...
!***
!*** I think things are working, it is just the nature of a stretch
!*** with very little deviation that is causing the painting in the browser to appear odd.  I am returning to the
!*** CENTURY version after getting the same results with the Savanna version.
!***
!*** R. Boone   Last updated: January 2, 2014  ... edited to store snow and snow liquid when the day length begins to decline ... too early?
   use Parameter_Vars
   use Structures
   implicit none
...
!   real     :: alat, dayl, dec, sinld, cosld, aob
   ...
   if (Rng(icell)%day_length .gt. Rng(icell)%last_month_day_length) then
     if (Rng(icell)%day_length_increasing .eqv. .FALSE.) then
