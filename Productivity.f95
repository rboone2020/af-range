...
! **** R. Boone   Last modified:  March 31, 2011
  ...
  tmin = Area(Rng(icell)%x, Rng(icell)%y)%min_temp
  tmax = Area(Rng(icell)%x, Rng(icell)%y)%max_temp
...
! Live biomass
  ...
  do ifacet = 1, FACETS
   ...
   !if (iunit .eq. 37) then
   !  write(*,*) 'Month        : ', month
   !  write(*,*) 'Obs Max Temp : ', Area(Rng(icell)%x, Rng(icell)%y)%max_temp           ! Only one month is loaded at a time, so no month index needed
   !  write(*,*) 'Obs Min Temp : ', Area(Rng(icell)%x, Rng(icell)%y)%min_temp
   !  write(*,*) 'TEMP MAX LEAF: ', temp_max_leaf
   !  write(*,*) 'TEMP MIN LEAF: ', temp_min_leaf
   !  write(*,*) 'TEMP MAX WOOD: ', temp_max_wood
   !  write(*,*) 'TEMP MIN WOOD: ', temp_min_wood
   !  write(*,*) 'Max Soil Temp: ', maximum_soil_surface_temperature
   !  write(*,*) 'Min Soil Temp: ', minimum_soil_surface_temperature
   !  write(*,*) 'Temp Min Melt: ', temp_min_melt
   !  write(*,*) 'Temp Max Melt: ', temp_max_melt
   !  write(*,*) 'Soil surftemp: ', Rng(icell)%soil_surface_temperature
   !  write(*,*) ' '
   !end if
  end do
...
!****                              Modified fairly extensively.   Now production is estimated, then allocated across layers.
!****                              Modified to avoid duplication production estimates across all the layers.
!****                              Experimenting with ways to allocate production across layers.
!****                              Change made to prevent Prop_Live_Per_Layer from running wild.
!****                              Removed incorrect assignment on co2 effect on root shoot ratio.  Leaf essentially 1.0 for now.
    use Parameter_Vars
    use Structures
    implicit none
    ...
    if ( Rng(icell)%pot_evap .ge. 0.01 ) then
      h2ogef = ( Rng(icell)%water_available(1) + Area(Rng(icell)%x, Rng(icell)%y)%precip ) / Rng(icell)%pot_evap         ! Irrigation was not included, unlike CENTURY
!     write(*,*) 'H2OGEF 1: ', icell, Rng(icell)%water_available(1), Area(Rng(icell)%x, Rng(icell)%y)%precip, Rng(icell)%pot_evap
    else
      h2ogef = 0.01
!     write(*,*) 'H2OGEF 2: Reset to 0.01'
    endif
...
!   write(*,*) 'H2OGEF 3: ', slope, h2ogef, Parms(iunit)%ppt_regression_points(3)
    if (h2ogef .gt. 1.0) then
      h2ogef = 1.0
!     write(*,*) 'H2OGEF 4: Reset to 1.0 in step 4'
    elseif (h2ogef .lt. 0.01) then
      h2ogef = 0.01
!     write(*,*) 'H2OGEF 5: Reset to 0.01 in step 5'
    endif

    ! Calculate how much live aboveground biomass is in each vegetation layer, and use that as a guide to distribute production
    ! NOW INCORPORATING FACETS INTO ALLOCATION
    ! Another attempt, using facet cover alone to allocate production.  Allow the shading multiplier et al. below to do their work.
    ! At most, if those had no effect, the production would be H_LYR + S_LYR + T_LYR + H_S_LYR + H_T_LYR + T_LYR
...
!      prop_live_per_layer(H_LYR) = Rng(icell)%facet_cover(H_FACET)
!      prop_live_per_layer(H_S_LYR) = Rng(icell)%facet_cover(S_FACET)
!      prop_live_per_layer(H_T_LYR) = Rng(icell)%facet_cover(T_FACET)
!      prop_live_per_layer(S_LYR) = Rng(icell)%facet_cover(S_FACET)
!      prop_live_per_layer(S_T_LYR) = Rng(icell)%facet_cover(T_FACET)
!      prop_live_per_layer(T_LYR) = Rng(icell)%facet_cover(T_FACET)
      ...
      do ilyr = 1, V_LYRS
        ...
        if (Rng(icell)%soil_surface_temperature .gt. 0.0) then
          ...
          a = Parms(iunit)%temperature_production(ifacet, 1)
          b = Parms(iunit)%temperature_production(ifacet, 2)
          c = Parms(iunit)%temperature_production(ifacet, 3)
          d = Parms(iunit)%temperature_production(ifacet, 4)
...
!if (Rng(icell)%range_type .eq. 37) then
!  write(*,*) '****** '
!  write(*,*) 'ZZ       A B C D: ', icell, ilyr, month, a, b, c, d
!  write(*,*) 'ZZ SoilSurf temp: ', icell, ilyr, month, Rng(icell)%soil_surface_temperature
!  write(*,*) 'ZZ          Frac: ', icell, ilyr, month, frac
!  write(*,*) 'ZZ Above -> Potential production, or P_P '
!  write(*,*) '  '
!  write(*,*) 'ZZ TOT_POT_PROD: ', icell, ilyr, month, Rng(icell)%total_pot_production(ilyr)
!  write(*,*) 'ZZ  total_cover: ', icell, ilyr, month, total_cover
...
!end if
    ...
    if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'POTENTL')
...
!  write(*,*) 'ZZ   TOT POT PROD ABOVE + BELOW:  ', icell, ilyr, agrd, bgrd, Rng(icell)%total_pot_production(ilyr)
   if (agrd .gt. 0.0) then
   ...
   if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'GRZ_RST')
...
!****                                          Edited to stop growth when phenology is 4.0
   ...
   if (Rng(icell)%ratio_water_pet > 0.0 .and. Area(Rng(icell)%x, Rng(icell)%y)%min_temp .gt. 3.0 .and. &
       Rng(icell)%phenology(H_FACET) .lt. 3.999 ) then
!    write(*,*) 'ZZ   PRE RESET OF PROD:  ', icell, Rng(icell)%ratio_water_pet, Area(Rng(icell)%x, Rng(icell)%y)%min_temp
!    write(*,*) 'ZZ                       ', Rng(icell)%phenology(H_FACET)
     ...
     do ilayer=1, SOIL_LAYERS                     ! Nutrients will be used to calculate mineral availablity for all layers, since they are only 15 cm each, 4, across the area.  Recent CENTURY uses a parameter (CLAYPG) here.
     ...
     if (avg_total_pot_prod_carbon .gt. 0.0) then         ! Wouldn't this be production limited by nitrogen?
       ...
       resp_temp_effect = 0.1 * exp(0.07 * Area(Rng(icell)%x,Rng(icell)%y)%temperature_average)
...
!    write(*,*) 'ZZ   RESETING BBB   TOT POT PROD  H_LYR:  ', icell, H_LYR, Rng(icell)%total_pot_production(H_LYR)
  ...
  if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'HRB_GRW')
...
! Not used, and causing error, and likely not needed.  Here, phenology is not regarding ages of woody plants.
! They are mixed woodlands, with shrubs or trees of many ages.  Phenology must be the focus for dormancy.
! Yes, not used.  GRange cells are spatial, and so diverse that speaking of "young forest" and "old forest" isn't appropriate.

   ! Century's site potential
   if (Area(Rng(icell)%x, Rng(icell)%y)%precip .lt. 20.0/6.0) then
      site_potential = 1500.0
   else if (Area(Rng(icell)%x, Rng(icell)%y)%precip .gt. 90.0/6.0) then
      site_potential = 3250.0
   else
       ! Line linear regression is not working.  Hardwiring equation
        site_potential = ( 3250.0 - 1500.0 ) / ( ( 90.0 / 6.0 ) - ( 20.0/ 6.0 ) ) * &
                        ( Area(Rng(icell)%x, Rng(icell)%y)%precip - (90.0 / 6.0 ) ) + 3250.0
   end if
 ...
 ! write(*,*) 'TM   :  ', tm
 ! write(*,*) 'S_P  :  ', site_potential
 ! write(*,*) 'TBAGN:  ', Parms(iunit)%tree_basal_area_to_grass_nitrogen
 ! write(*,*) 'AREA:   ', Rng(icell)%tree_basal_area
 ! write(*,*) 'A_N   : ', available_nitrogen
  ...
  if (gnfrac .lt. 0.0 .or. gnfrac .gt. 1.0) gnfrac = 0.0
   ...
   if (Rng(icell)%ratio_water_pet > 0.0 .and. Area(Rng(icell)%x, Rng(icell)%y)%min_temp .gt. 0.0 ) then
...
!        write(*,*) 'ZZ   TOT POT PROD CCCCC  ', icell, ifacet, 'Edited for annual deciduous '
       end if
     end do
   else
...
!    write(*,*) 'ZZ   RESET TOT POT PROD DDDDD  ', icell, 'Resetting shrub and tree layers.'
   end if
...
!    write(*,*) 'ZZ   AVERAGE POT PRODUCTION: ', icell, avg_pot_production
     ...
     if (avg_pot_production .gt. 0.0) then
       ...
       !****** NOT SURE IF THE FOLLOWING IS CORRECT.   IF CORRECT, MAY BE EXPANDED UPON
       !****** CHANGE THE NAME TO SPEAK OF FRACTION CARBON TO FINE ROOTS????
...
!      write(*,*) 'ZZ   TOT POT PROD  EEEE   Woody parts reset to 0.0'
     end if

   end do

   if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'WDY_GRW')
...
!**** R. Boone                 Last modified:  February 16, 2013
   ...
   !write(*,*) 'FACET:  ', ifacet
   !write(*,*) 'A_N: ', available_nitrogen
   !write(*,*) 'PARM:  ', Parms(iunit)%fraction_nitrogen_available
   !write(*,*) 'RIMP:   ', rimpct
   !write(*,*) 'STORED: ', Rng(icell)%stored_nitrogen(ifacet)
...
!     min_n = min_n + ( cfrac(ipart) * min_n_ci(ipart) )                        ! An error in Century, with MIN and MAX flipped?  Lines 102,103 in RESTRP.F  No, correct.  But MIN_N only used in Century in automatic fertilization, so skipped.
   end do
...
!   max_n = max_n * ctob      ! Skipping this for now (counter to CENTURY RSTRP.F).  So MAX_N is being passed as a weighted nitrogen concentration.   Converting what I have to biomass doesn't make any sense.
   ...
   if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'RESTRCT')
...
!****                          Both minor and more significant corrections from the Feb. 2013 version.
!****                          Restored biomass to N conversion with ecfor and cpbe.  Ecfor is in biomass units, but need "carbon" or nitrogen units.
!****                          Rewrite modifying the way total_pot_prod_limited_by_n is calculated.   It is not an index.
!****                          Streamlining some, moving away from CENTURY with its multiple nutrients, so a simplier construction
!****                          Aligning logic of Nutrient_Limitation with G-Range, modifying Century code as needed
   use Parameter_Vars
   use Structures
   implicit none
   ...
   !write (*,*) 'maxN' , max_n_fix
   !write (*,*) 'totP' , total_pot_production_carbon
   !write (*, *) 'AvaN' , available_nitrogen
...
!       The following is part of the DEBUG, to avoid errors    ! DEBUG
     ...
     do ipart = 1, nparts
     !write(*,*) 'IPART:  ', ipart
     !write(*,*) 'MIN 2:   ', min_n_ci(ipart)
     !write(*,*) 'MAX 2:  ', max_n_ci(ipart)
     !write(*,*) 'TOTAL:       ', totaln
     !write(*,*) 'DEMAND:  ', demand
...
!       The following is part of the DEBUG, to avoid errors    ! DEBUG
   ...
   if (total_pot_production_biomass .gt. cpbe) then
     !if (totaln_used .lt. demand) then
     !  write(*,*) 'LIMITS:  ', icell, ifacet, total_pot_production_biomass, totaln_used, demand
     !end if
   ...
   do ipart = 1,nparts
    !write(*,*) 'CELL IFACET IPART: ', icell, ifacet, ipart
    !write(*,*) 'T_P_P_C: ', total_pot_production_carbon
    !write(*,*) 'CFRAC:     ', cfrac(ipart)
    !write(*,*) 'IPART:       ', ipart
    !write(*,*) 'ECFOR:     ', ecfor(ipart)
    !write (*,*) ' '
   ...
   if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'NUT_LIM')
...
!**** R. Boone                 Last modified:  October 5, 2013 ... changed * to + in optimum leaf area index calculation
   ...
   if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'LEAF_AL')

end function


! THE CENTURY GRAZING ROUTINE WILL NOT BE USED. INSTEAD, A MORE DETAILED TREATMENT BASED ON HERBIVORE GRAZING FROM EIGHT BIOMASS POOLS WILL BE USED.
...
!
!   if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'GRAZING')
!
!end subroutine


subroutine Grazing (icell)
!**** Remove forage that is grazed based on the grazing of herbivores simulated in each cell and eating from eight pools.
!****
!**** R. Boone                 Last modified:  June 21, 2023
   use Parameter_Vars
   use Structures
   implicit none
   ...
   total_nitrogen = 0.0
 ! GREEN HERB LEAVES
   if (Rng(icell)%grazed_green_herb_fraction .gt. 0.0 .and. Rng(icell)%leaf_carbon(H_FACET) .gt. 0.0) then
     carbon_removed = Rng(icell)%leaf_carbon(H_FACET) * Rng(icell)%grazed_green_herb_fraction
   else
     carbon_removed = 0.0
   end if
   if (Rng(icell)%grazed_green_herb_fraction .gt. 0.0 .and. Rng(icell)%leaf_nitrogen(H_FACET) .gt. 0.0) then
     nitrogen_removed = Rng(icell)%leaf_nitrogen(H_FACET) * Rng(icell)%grazed_green_herb_fraction
   else
     nitrogen_removed = 0.0
   end if
   Rng(icell)%leaf_carbon(H_FACET) = Rng(icell)%leaf_carbon(H_FACET) - carbon_removed
   Rng(icell)%leaf_nitrogen(H_FACET) = Rng(icell)%leaf_nitrogen(H_FACET) - nitrogen_removed
   Rng(icell)%carbon_source_sink = Rng(icell)%carbon_source_sink + carbon_removed
   Rng(icell)%nitrogen_source_sink = Rng(icell)%nitrogen_source_sink + nitrogen_removed
   total_carbon = total_carbon + carbon_removed
   total_nitrogen = total_nitrogen + nitrogen_removed
 ! GREEN HERB SEEDS
   if (Rng(icell)%grazed_green_herb_fraction .gt. 0.0 .and. Rng(icell)%seed_carbon(H_FACET) .gt. 0.0) then
     carbon_removed = Rng(icell)%seed_carbon(H_FACET) * Rng(icell)%grazed_green_herb_fraction
   else
     carbon_removed = 0.0
   end if
   if (Rng(icell)%grazed_green_herb_fraction .gt. 0.0 .and. Rng(icell)%seed_nitrogen(H_FACET) .gt. 0.0) then
     nitrogen_removed = Rng(icell)%seed_nitrogen(H_FACET) * Rng(icell)%grazed_green_herb_fraction
   else
     nitrogen_removed = 0.0
   end if
   Rng(icell)%seed_carbon(H_FACET) = Rng(icell)%seed_carbon(H_FACET) - carbon_removed
   Rng(icell)%seed_nitrogen(H_FACET) = Rng(icell)%seed_nitrogen(H_FACET) - nitrogen_removed
   Rng(icell)%carbon_source_sink = Rng(icell)%carbon_source_sink + carbon_removed
   Rng(icell)%nitrogen_source_sink = Rng(icell)%nitrogen_source_sink + nitrogen_removed
   total_carbon = total_carbon + carbon_removed
   total_nitrogen = total_nitrogen + nitrogen_removed
 ! DEAD HERB
   if (Rng(icell)%grazed_dead_herb_fraction .gt. 0.0 .and. Rng(icell)%dead_standing_carbon(H_FACET) .gt. 0.0) then
     carbon_removed = Rng(icell)%dead_standing_carbon(H_FACET) * Rng(icell)%grazed_dead_herb_fraction
   else
     carbon_removed = 0.0
   end if
   if (Rng(icell)%grazed_dead_herb_fraction .gt. 0.0 .and. Rng(icell)%dead_standing_nitrogen(H_FACET) .gt. 0.0) then
     nitrogen_removed = Rng(icell)%dead_standing_nitrogen(H_FACET) * Rng(icell)%grazed_dead_herb_fraction
   else
     nitrogen_removed = 0.0
   end if
   Rng(icell)%dead_standing_carbon(H_FACET) = Rng(icell)%dead_standing_carbon(H_FACET) - carbon_removed
   Rng(icell)%dead_standing_nitrogen(H_FACET) = Rng(icell)%dead_standing_nitrogen(H_FACET) - nitrogen_removed
   Rng(icell)%carbon_source_sink = Rng(icell)%carbon_source_sink + carbon_removed
   Rng(icell)%nitrogen_source_sink = Rng(icell)%nitrogen_source_sink + nitrogen_removed
   total_carbon = total_carbon + carbon_removed
   total_nitrogen = total_nitrogen + nitrogen_removed
 ! GREEN SHRUB LEAVES
   if (Rng(icell)%grazed_green_shrub_fraction .gt. 0.0 .and. Rng(icell)%leaf_carbon(S_FACET) .gt. 0.0) then
     carbon_removed = Rng(icell)%leaf_carbon(S_FACET) * Rng(icell)%grazed_green_shrub_fraction
   else
     carbon_removed = 0.0
   end if
   if (Rng(icell)%grazed_green_shrub_fraction .gt. 0.0 .and. Rng(icell)%leaf_nitrogen(S_FACET) .gt. 0.0) then
     nitrogen_removed = Rng(icell)%leaf_nitrogen(S_FACET) * Rng(icell)%grazed_green_shrub_fraction
   else
     nitrogen_removed = 0.0
   end if
   Rng(icell)%leaf_carbon(S_FACET) = Rng(icell)%leaf_carbon(S_FACET) - carbon_removed
   Rng(icell)%leaf_nitrogen(S_FACET) = Rng(icell)%leaf_nitrogen(S_FACET) - nitrogen_removed
   Rng(icell)%carbon_source_sink = Rng(icell)%carbon_source_sink + carbon_removed
   Rng(icell)%nitrogen_source_sink = Rng(icell)%nitrogen_source_sink + nitrogen_removed
   total_carbon = total_carbon + carbon_removed
   total_nitrogen = total_nitrogen + nitrogen_removed
 ! GREEN SHRUB SEEDS
   if (Rng(icell)%grazed_green_shrub_fraction .gt. 0.0 .and. Rng(icell)%seed_carbon(S_FACET) .gt. 0.0) then
     carbon_removed = Rng(icell)%seed_carbon(S_FACET) * Rng(icell)%grazed_green_shrub_fraction
   else
     carbon_removed = 0.0
   end if
   if (Rng(icell)%grazed_green_shrub_fraction .gt. 0.0 .and. Rng(icell)%seed_nitrogen(S_FACET) .gt. 0.0) then
     nitrogen_removed = Rng(icell)%seed_nitrogen(S_FACET) * Rng(icell)%grazed_green_shrub_fraction
   else
     nitrogen_removed = 0.0
   end if
   Rng(icell)%seed_carbon(S_FACET) = Rng(icell)%seed_carbon(S_FACET) - carbon_removed
   Rng(icell)%seed_nitrogen(S_FACET) = Rng(icell)%seed_nitrogen(S_FACET) - nitrogen_removed
   Rng(icell)%carbon_source_sink = Rng(icell)%carbon_source_sink + carbon_removed
   Rng(icell)%nitrogen_source_sink = Rng(icell)%nitrogen_source_sink + nitrogen_removed
   total_carbon = total_carbon + carbon_removed
   total_nitrogen = total_nitrogen + nitrogen_removed
 ! DEAD SHRUB LEAVES
   if (Rng(icell)%grazed_dead_shrub_fraction .gt. 0.0 .and. Rng(icell)%dead_standing_carbon(S_FACET) .gt. 0.0) then
     carbon_removed = Rng(icell)%dead_standing_carbon(S_FACET) * Rng(icell)%grazed_dead_shrub_fraction
   else
     carbon_removed = 0.0
   end if
   if (Rng(icell)%grazed_dead_shrub_fraction .gt. 0.0 .and. Rng(icell)%dead_standing_nitrogen(S_FACET) .gt. 0.0) then
     nitrogen_removed = Rng(icell)%dead_standing_nitrogen(S_FACET) * Rng(icell)%grazed_dead_shrub_fraction
   else
     nitrogen_removed = 0.0
   end if
   Rng(icell)%dead_standing_carbon(S_FACET) = Rng(icell)%dead_standing_carbon(S_FACET) - carbon_removed
   Rng(icell)%dead_standing_nitrogen(S_FACET) = Rng(icell)%dead_standing_nitrogen(S_FACET) - nitrogen_removed
   Rng(icell)%carbon_source_sink = Rng(icell)%carbon_source_sink + carbon_removed
   Rng(icell)%nitrogen_source_sink = Rng(icell)%nitrogen_source_sink + nitrogen_removed
   total_carbon = total_carbon + carbon_removed
   total_nitrogen = total_nitrogen + nitrogen_removed
 ! FINE BRANCH SHRUB
   if (Rng(icell)%grazed_fbranch_shrub_fraction .gt. 0.0 .and. Rng(icell)%fine_branch_carbon(S_FACET) .gt. 0.0) then
     carbon_removed = Rng(icell)%fine_branch_carbon(S_FACET) * Rng(icell)%grazed_fbranch_shrub_fraction
   else
     carbon_removed = 0.0
   end if
   if (Rng(icell)%grazed_fbranch_shrub_fraction .gt. 0.0 .and. Rng(icell)%fine_branch_nitrogen(S_FACET) .gt. 0.0) then
     nitrogen_removed = Rng(icell)%fine_branch_nitrogen(S_FACET) * Rng(icell)%grazed_fbranch_shrub_fraction
   else
     nitrogen_removed = 0.0
   end if
   Rng(icell)%fine_branch_carbon(S_FACET) = Rng(icell)%fine_branch_carbon(S_FACET) - carbon_removed
   Rng(icell)%fine_branch_nitrogen(S_FACET) = Rng(icell)%fine_branch_nitrogen(S_FACET) - nitrogen_removed
   Rng(icell)%carbon_source_sink = Rng(icell)%carbon_source_sink + carbon_removed
   Rng(icell)%nitrogen_source_sink = Rng(icell)%nitrogen_source_sink + nitrogen_removed
   total_carbon = total_carbon + carbon_removed
   total_nitrogen = total_nitrogen + nitrogen_removed
 ...
 ! GREEN TREE LEAVES
   if (Rng(icell)%grazed_green_tree_fraction .gt. 0.0 .and. Rng(icell)%leaf_carbon(T_FACET) .gt. 0.0) then
     carbon_removed = Rng(icell)%leaf_carbon(T_FACET) * Rng(icell)%grazed_green_tree_fraction
   else
     carbon_removed = 0.0
   end if
   if (Rng(icell)%grazed_green_tree_fraction .gt. 0.0 .and. Rng(icell)%leaf_nitrogen(T_FACET) .gt. 0.0) then
     nitrogen_removed = Rng(icell)%leaf_nitrogen(T_FACET) * Rng(icell)%grazed_green_tree_fraction
   else
     nitrogen_removed = 0.0
   end if
   Rng(icell)%leaf_carbon(T_FACET) = Rng(icell)%leaf_carbon(T_FACET) - carbon_removed
   Rng(icell)%leaf_nitrogen(T_FACET) = Rng(icell)%leaf_nitrogen(T_FACET) - nitrogen_removed
   Rng(icell)%carbon_source_sink = Rng(icell)%carbon_source_sink + carbon_removed
   Rng(icell)%nitrogen_source_sink = Rng(icell)%nitrogen_source_sink + nitrogen_removed
   total_carbon = total_carbon + carbon_removed
   total_nitrogen = total_nitrogen + nitrogen_removed
 ! GREEN TREE SEEDS
   if (Rng(icell)%grazed_green_tree_fraction .gt. 0.0 .and. Rng(icell)%seed_carbon(T_FACET) .gt. 0.0) then
     carbon_removed = Rng(icell)%seed_carbon(T_FACET) * Rng(icell)%grazed_green_tree_fraction
   else
     carbon_removed = 0.0
   end if
   if (Rng(icell)%grazed_green_tree_fraction .gt. 0.0 .and. Rng(icell)%seed_nitrogen(T_FACET) .gt. 0.0) then
     nitrogen_removed = Rng(icell)%seed_nitrogen(T_FACET) * Rng(icell)%grazed_green_tree_fraction
   else
     nitrogen_removed = 0.0
   end if
   Rng(icell)%seed_carbon(T_FACET) = Rng(icell)%seed_carbon(T_FACET) - carbon_removed
   Rng(icell)%seed_nitrogen(T_FACET) = Rng(icell)%seed_nitrogen(T_FACET) - nitrogen_removed
   Rng(icell)%carbon_source_sink = Rng(icell)%carbon_source_sink + carbon_removed
   Rng(icell)%nitrogen_source_sink = Rng(icell)%nitrogen_source_sink + nitrogen_removed
   total_carbon = total_carbon + carbon_removed
   total_nitrogen = total_nitrogen + nitrogen_removed
 ! DEAD TREE LEAVES
   if (Rng(icell)%grazed_dead_tree_fraction .gt. 0.0 .and. Rng(icell)%dead_standing_carbon(T_FACET) .gt. 0.0) then
     carbon_removed = Rng(icell)%dead_standing_carbon(T_FACET) * Rng(icell)%grazed_dead_tree_fraction
   else
     carbon_removed = 0.0
   end if
   if (Rng(icell)%grazed_dead_tree_fraction .gt. 0.0 .and. Rng(icell)%dead_standing_nitrogen(T_FACET) .gt. 0.0) then
     nitrogen_removed = Rng(icell)%dead_standing_nitrogen(T_FACET) * Rng(icell)%grazed_dead_tree_fraction
   else
     nitrogen_removed = 0.0
   end if
   Rng(icell)%dead_standing_carbon(T_FACET) = Rng(icell)%dead_standing_carbon(T_FACET) - carbon_removed
   Rng(icell)%dead_standing_nitrogen(T_FACET) = Rng(icell)%dead_standing_nitrogen(T_FACET) - nitrogen_removed
   Rng(icell)%carbon_source_sink = Rng(icell)%carbon_source_sink + carbon_removed
   Rng(icell)%nitrogen_source_sink = Rng(icell)%nitrogen_source_sink + nitrogen_removed
   total_carbon = total_carbon + carbon_removed
   total_nitrogen = total_nitrogen + nitrogen_removed
 ! FINE BRANCH TREE
   if (Rng(icell)%grazed_fbranch_tree_fraction .gt. 0.0 .and. Rng(icell)%fine_branch_carbon(T_FACET) .gt. 0.0) then
     carbon_removed = Rng(icell)%fine_branch_carbon(T_FACET) * Rng(icell)%grazed_fbranch_tree_fraction
   else
     carbon_removed = 0.0
   end if
   if (Rng(icell)%grazed_fbranch_tree_fraction .gt. 0.0 .and. Rng(icell)%fine_branch_nitrogen(T_FACET) .gt. 0.0) then
     nitrogen_removed = Rng(icell)%fine_branch_nitrogen(T_FACET) * Rng(icell)%grazed_fbranch_tree_fraction
   else
     nitrogen_removed = 0.0
   end if
   Rng(icell)%fine_branch_carbon(T_FACET) = Rng(icell)%fine_branch_carbon(T_FACET) - carbon_removed
   Rng(icell)%fine_branch_nitrogen(T_FACET) = Rng(icell)%fine_branch_nitrogen(T_FACET) - nitrogen_removed
   Rng(icell)%carbon_source_sink = Rng(icell)%carbon_source_sink + carbon_removed
   Rng(icell)%nitrogen_source_sink = Rng(icell)%nitrogen_source_sink + nitrogen_removed
   total_carbon = total_carbon + carbon_removed
   total_nitrogen = total_nitrogen + nitrogen_removed
