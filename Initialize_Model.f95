...
!**** R. Boone    Last modified: April 18, 2023 - to incorporate herbivores
!****                            January 20, 2025 - to incorporate plant coefficients affecting establishment and death
!****                            April 10, 2026 - temperature production coefficients made to be facet specific
   use Parameter_Vars
   use Structures
   implicit none
   ...
   write(*,*)
   write(*,*) '    AF-RANGE Ver 1.00  May 22, 2026'
   write(*,*)
   ! ***************************************************
   ! ***************************************************
   ...
   if (err_code == 0) then
     write(*,*) 'The path to the binary files is: ', trim(bin_path)
   else
     write(*,*) 'The path to the binary files cannot be determined.'
     stop
   end if
   ...
   if (ioerr == 0) then
     ...
     write(*,*) 'The application path cannot be read from the AF_RANGE.INI file in the program directory: ', &
     bin_path(1:len_trim(bin_path))//INI_FILE
     stop
   end if
   close(SHORT_USE_FILE)

   ! Open an echo file, which will remain open throughout the simulation, and accept any typeof echoed information.
   ...
   if (ioerr == 0) then
     ...
     write(ECHO_FILE,*) 'L-RANGE ECHO FILE - Capturing echoed inputs and other information'
     write(ECHO_FILE,*) ' '
     write(ECHO_FILE,*) 'The path to the binary file is: ', trim(bin_path)
     write(ECHO_FILE,*) 'The application pathway is:     ', trim(app_path)
     write(ECHO_FILE,*) 'The pathway to this file is:    ', app_path(1:len_trim(app_path))//ECHO_NAME
   else
   ...
   if (ioerr == 0) then
     ...
     read(SHORT_USE_FILE,*) Sim_Parm%area_map
     write(ECHO_FILE,*) 'The area map showing areas of study versus background: ', trim(Sim_Parm%area_map)
...
!     Sim_Parm%precip_path_prefix = app_path(1:len_trim(app_path))//Sim_Parm%precip_path_prefix
...
!     Sim_Parm%max_temp_path_prefix = app_path(1:len_trim(app_path))//Sim_Parm%max_temp_path_prefix
...
!     Sim_Parm%min_temp_path_prefix = app_path(1:len_trim(app_path))//Sim_Parm%min_temp_path_prefix
     ...
     read(SHORT_USE_FILE,*) Sim_Parm%herbivore_file_name
     write(ECHO_FILE,*) 'The file name storing the parameters controlling herbivores: ', Sim_Parm%herbivore_file_name
...
subroutine Initialize_Herbivore_Parms
!**** Initialize_Herbivore_Parms loads the parameters that manage herbivore foraging and dynamics.
!**** R. Boone    Last modified: March 21, 2025.  Added a density multiplier.
   use Parameter_Vars
   use Structures
   implicit none
   integer i, ispp, j

   ! ----  PROCESS THE HERBIVORE PARAMETER FILE  ----
   open(SHORT_USE_FILE, FILE=app_path(1:len_trim(app_path))//'\Parms\'//Sim_Parm%herbivore_file_name, ACTION='READ', IOSTAT=ioerr)
   if (ioerr == 0) then
     read(SHORT_USE_FILE,*) Herbivore_Parms%herbivores
     write(ECHO_FILE,*) 'The number of herbivore species included: ', Herbivore_Parms%herbivores
     herbivores = Herbivore_Parms%herbivores                                         ! for brevity
     do ispp=1,herbivores
       read(SHORT_USE_FILE,*) Herbivore_Parms%spp_name(ispp)
       write(ECHO_FILE,*) 'The species include ', ispp, '  ', Herbivore_parms%spp_name(ispp)
     end do
     do ispp=1,herbivores
       read(SHORT_USE_FILE,*) Herbivore_Parms%spp_map(ispp)
       write(ECHO_FILE,*) 'The species maps include ', ispp, '  ', Herbivore_parms%spp_map(ispp)
     end do
     read(SHORT_USE_FILE,*) Herbivore_Parms%prop_forage_avail_map
     write(ECHO_FILE,*) 'The map storing proportion forage available: ', Herbivore_Parms%prop_forage_avail_map
     read(SHORT_USE_FILE,*) (Herbivore_Parms%model_dynamics(ispp), ispp=1,herbivores)
     write(ECHO_FILE,*) 'Model dynamics (2) or not (1) for species: ',(Herbivore_Parms%model_dynamics(ispp), ispp=1,herbivores)
     write(*,*) 'Note: If herbivore dynamics are being simulated, populations will be limited to a multiplier of ', &
          'observed densities for each landscape unit.'
     read(SHORT_USE_FILE,*) Herbivore_Parms%cell_resolution
     write(ECHO_FILE,*) 'Cell resolution for application: ', Herbivore_Parms%cell_resolution
     do ispp=1,herbivores
       read(SHORT_USE_FILE,*) (Herbivore_Parms%min_max_mass_ratio(ispp,j), j=1,2)
       write(ECHO_FILE,*) 'Min max body mass for spp: ', ispp, '   ', (Herbivore_Parms%min_max_mass_ratio(ispp,j), j=1,2)
       read(SHORT_USE_FILE,*) Herbivore_Parms%expected_kg(ispp)
       write(ECHO_FILE,*) 'Expected body mass for adult: ', ispp, '  ', Herbivore_Parms%expected_kg(ispp)
       read(SHORT_USE_FILE,*) (Herbivore_Parms%min_max_mass_change(ispp,j), j=1,2)
       write(ECHO_FILE,*) 'Min max mass change for spp: ', ispp, '   ', (Herbivore_Parms%min_max_mass_change(ispp,j), j=1,2)
       read(SHORT_USE_FILE,*) (Herbivore_Parms%min_max_energy_use(ispp,j), j=1,2)
       write(ECHO_FILE,*) 'Min max energy use for spp:  ', ispp, '   ', (Herbivore_Parms%min_max_energy_use(ispp,j), j=1,2)
       read(SHORT_USE_FILE,*) (Herbivore_Parms%max_energy_use_vs_ci(ispp,j), j=1,6)
       write(ECHO_FILE,*) 'Max energy use vs CI for spp: ', ispp, '  ', (Herbivore_Parms%max_energy_use_vs_ci(ispp,j), j=1,6)
       read(SHORT_USE_FILE,*) Herbivore_Parms%daily_offtake_prop(ispp)
       write(ECHO_FILE,*) 'Daily offtake prop for spp: ', ispp, '  ', Herbivore_Parms%daily_offtake_prop(ispp)
       read(SHORT_USE_FILE,*) Herbivore_Parms%metabilizability(ispp)
       write(ECHO_FILE,*) 'Metalizability for spp: ', ispp, '  ', Herbivore_Parms%metabilizability(ispp)
       read(SHORT_USE_FILE,*) (Herbivore_Parms%max_intake_rate(ispp,j), j=1,8)
       write(ECHO_FILE,*) 'Max intake rate for spp: ', ispp, '  ', (Herbivore_Parms%max_intake_rate(ispp,j), j=1,8)
       read(SHORT_USE_FILE,*) (Herbivore_Parms%pool_preference(ispp,j), j=1,8)
       write(ECHO_FILE,*) 'Pool preference for spp: ', ispp, '  ', (Herbivore_Parms%pool_preference(ispp,j), j=1,8)
       read(SHORT_USE_FILE,*) (Herbivore_Parms%digestibility_fraction(ispp,j), j=1,8)
       write(ECHO_FILE,*) 'Digestibility fraction for spp: ', ispp, '  ', (Herbivore_Parms%digestibility_fraction(ispp,j), j=1,8)
       read(SHORT_USE_FILE,*) (Herbivore_Parms%ungrazable_biomass(ispp,j), j=1,2)
       write(ECHO_FILE,*) 'Ungrazable biomass, life dead for spp: ', ispp, '  ',(Herbivore_Parms%ungrazable_biomass(ispp,j), j=1,2)
       read(SHORT_USE_FILE,*) Herbivore_Parms%wasted_prop(ispp)
       write(ECHO_FILE,*) 'Wasted proportion of forage for spp: ', ispp, '  ', Herbivore_Parms%wasted_prop(ispp)
       read(SHORT_USE_FILE,*) (Herbivore_Parms%critical_temps(ispp,j), j=1,3)
       write(ECHO_FILE,*) 'Critical temperatures for spp: ', ispp, '  ', (Herbivore_Parms%critical_temps(ispp,j), j=1,3)
       read(SHORT_USE_FILE,*) Herbivore_Parms%thermal_cost_prop(ispp)
       write(ECHO_FILE,*) 'Thermal cost proportion for spp: ', ispp, '  ', Herbivore_Parms%thermal_cost_prop(ispp)
       read(SHORT_USE_FILE,*) (Herbivore_Parms%birth_rate_vs_ci(ispp,j), j=1,8)
       write(ECHO_FILE,*) 'Birth rate vs CI for spp: ', ispp, '  ', (Herbivore_Parms%birth_rate_vs_ci(ispp,j), j=1,8)
       read(SHORT_USE_FILE,*) (Herbivore_Parms%death_rate_vs_ci(ispp,j), j=1,8)
       write(ECHO_FILE,*) 'Death rate vs CI for spp: ', ispp, '  ', (Herbivore_Parms%death_rate_vs_ci(ispp,j), j=1,8)
       read(SHORT_USE_FILE,*) Herbivore_Parms%max_density_multiplier(ispp)
       write(ECHO_FILE,*) 'Herbivore maximum density multiplier: ', ispp, Herbivore_Parms%max_density_multiplier(ispp)
     end do
   else
     write(ECHO_FILE,*) 'The main parameter file cannot be opened: ',app_path(1:len_trim(app_path))//Sim_Parm%herbivore_file_name
   end if
   close(SHORT_USE_FILE)
...
subroutine Initialize_Herbivore_Pops
!**** The herbivore populations will be read into the model landscape cells.
!**** The program will uses GRID ASCII files.
!****
!**** R. Boone    Last modified: May 22, 2023
   use Parameter_Vars
   use Structures
   implicit none
   integer a_val, ix, iy, i, rowarr(MAX_X_DIM), ispp
   character(12) a_string
   character(80) big_string
   character(100) forage_string

   herbivores = Herbivore_Parms%herbivores

   ! Read in the herbivore maps
   do ispp=1,herbivores
     write(*,*) 'Reading in the map for species: ', ispp, '  ', Herbivore_Parms%spp_name(ispp)
     if (Herbivore_Parms%model_dynamics(ispp) .eq. 0) then
       write(*,*) '*** Note: Populations for species ', ispp, ' are set to zero. ***'
     end if
     call Read_Map ('Species map ', Herbivore_Parms%spp_map(ispp))
     ! Transfer the map
     do iy=1,y_dim
       do ix=1,x_dim
         if (Herbivore_Parms%model_dynamics(ispp) .gt. 0) then
           Area(ix,iy)%herbivore_pop(ispp) = Area(ix,iy)%temporary
         else
           Area(ix,iy)%herbivore_pop(ispp) = 0
         end if
       end do
     end do
     ! Echo the map
     if (Sim_Parm%echo_maps .eqv. .true.) then
       write(ECHO_FILE,*) 'The map for species: ', ispp
       do iy=1,y_dim
         write(ECHO_FILE,1001) (Area(ix,iy)%herbivore_pop(ispp), ix=1,x_dim)
       end do
     end if
   end do
1001  format(5000L1)                              ! This value is hardwired

   ! Load the forage availability per cell map.  Recall this is annual and values will be divided by 12.
   call Read_Map ('Forage_avail', Herbivore_Parms%prop_forage_avail_map)
   ! Transfer the map
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%prop_forage_avail = Area(ix,iy)%temporary
       else
         Area(ix,iy)%prop_forage_avail = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     write(ECHO_FILE,*) 'The herbaceous ANNUAL forage availability map.'
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%prop_forage_avail)
       end do
...
subroutine Initialize_Landscape
!**** Initialize_Landscape reads the initial spatial data supporting L-Range.
!**** The program will uses GRID ASCII files.
!****
!**** R. Boone    Last modified: June 24, 2018
   use Parameter_Vars
   use Structures
   implicit none
   ...
   if (ioerr == 0) then
     ...
     read(SHORT_USE_FILE,*) a_string, a_val          ! Discard the null value identifier
     ! Read the data
     do iy=1,y_dim
       read(SHORT_USE_FILE,*) (Area(ix,iy)%zone, ix=1,x_dim)
     end do
   ...
   ! Read in the STUDY AREA map
   write(*,*) 'Reading in the study area map.'
   call Read_Map ('area or not ', Sim_Parm%Area_map)
   ! Transfer the map
   do iy=1,y_dim
     do ix=1,x_dim
       if (int(Area(ix,iy)%temporary) .ne. 0) then
         Area(ix,iy)%area = .true.
       else
         Area(ix,iy)%area = .false.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     write(ECHO_FILE,*) 'The study area map.'
     do iy=1,y_dim
       write(ECHO_FILE,1001) (Area(ix,iy)%area,ix=1,x_dim)
     end do
   end if
1001  format(5000L1)                              ! This value is hardwired
   ...
   ! NOTE NOTE NOTE
   ! NOTE:  ZERO VALUES ARE NOT PART OF THE STUDY AREA.  But all maps are rectangular
   ! NOTE:  Any area may be included as land cover.
   ! NOTE NOTE NOTE
   write(*,*) 'Reading in the land cover map.  Zero values will not be simulated.'
   call Read_Map ('land cover  ', Sim_Parm%class_map)
   ! Transfer the map
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary .lt. 0) then
         Area(ix,iy)%class = 0
       else
         Area(ix,iy)%class = Area(ix,iy)%temporary
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         if (Area(ix,iy)%class .gt. 0) then
           rowarr(ix) = Area(ix,iy)%class / 10
...
1002  format(5000I1)                                  ! This value is hardwired.
   ...
   ! Fill a global map with rangeland values
   do iy=1,y_dim
     do ix=1,x_dim
       i=Area(ix,iy)%class
       Area(ix,iy)%rangeland = .FALSE.
       if (i>0) then
         if (rangeland_classes(i) .eqv. .TRUE.) then
           Area(ix,iy)%rangeland = .TRUE.
   ...
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     write(ECHO_FILE,*) 'The rangeland map, with rangelands as 1 and others as 0:'
     do iy=1,y_dim
       do ix=1,x_dim
         if (Area(ix,iy)%rangeland .eqv. .TRUE.) then
           rowarr(ix) = 1
         else
           rowarr(ix) = 0
         end if
   ...
   write(ECHO_FILE,*) 'The number of rangeland cells across the area at this resolution is: ', range_cells
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       Area(ix,iy)%latitude = Area(ix,iy)%temporary
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = Area(ix,iy)%latitude / 10
       end do
...
1004  format(5000I2)                                  ! This value is hardwired.
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%top_sand = Area(ix,iy)%temporary              ! Percent
       else
         Area(ix,iy)%top_sand = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%top_sand / 10. )
       end do
       write(ECHO_FILE,1002) (rowarr(ix),ix=1,x_dim)
     end do
   end if
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%top_silt = Area(ix,iy)%temporary                     ! Percent
       else
         Area(ix,iy)%top_silt = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%top_silt / 10. )
       end do
       write(ECHO_FILE,1002) (rowarr(ix),ix=1,x_dim)
     end do
   end if
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%top_clay = Area(ix,iy)%temporary               ! Percent
       else
         Area(ix,iy)%top_clay = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%top_clay / 10. )
       end do
       write(ECHO_FILE,1002) (rowarr(ix),ix=1,x_dim)
     end do
   end if
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%top_gravel = Area(ix,iy)%temporary                 ! Percent
       else
         Area(ix,iy)%top_gravel = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%top_gravel / 10. )
       end do
       write(ECHO_FILE,1002) (rowarr(ix),ix=1,x_dim)
     end do
   end if
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%top_bulk_density = Area(ix,iy)%temporary
       else
         Area(ix,iy)%top_bulk_density = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%top_bulk_density )
         if (rowarr(ix) > 9) then
           rowarr(ix) = 9
         end if
       end do
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%top_organic_carbon = Area(ix,iy)%temporary
       else
         Area(ix,iy)%top_organic_carbon = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%top_organic_carbon )
         if (rowarr(ix) > 9) then
           rowarr(ix) = 9
         end if
       end do
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%sub_sand = Area(ix,iy)%temporary                   ! Percent
       else
         Area(ix,iy)%sub_sand = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%sub_sand / 10. )
       end do
       write(ECHO_FILE,1002) (rowarr(ix),ix=1,x_dim)
     end do
   end if
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%sub_silt = Area(ix,iy)%temporary               ! Percent
       else
         Area(ix,iy)%sub_silt = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%sub_silt / 10. )
       end do
       write(ECHO_FILE,1002) (rowarr(ix),ix=1,x_dim)
     end do
   end if
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%sub_clay = Area(ix,iy)%temporary                    ! Percent
       else
         Area(ix,iy)%sub_clay = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%sub_clay / 10. )
       end do
       write(ECHO_FILE,1002) (rowarr(ix),ix=1,x_dim)
     end do
   end if
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%sub_gravel = Area(ix,iy)%temporary                ! Percent
       else
         Area(ix,iy)%sub_gravel = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%sub_gravel / 10. )
       end do
       write(ECHO_FILE,1002) (rowarr(ix),ix=1,x_dim)
     end do
   end if
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%sub_bulk_density = Area(ix,iy)%temporary
       else
         Area(ix,iy)%sub_bulk_density = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%sub_bulk_density )
         if (rowarr(ix) > 9) then
           rowarr(ix) = 9
         end if
       end do
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%sub_organic_carbon = Area(ix,iy)%temporary
       else
         Area(ix,iy)%sub_organic_carbon = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%sub_organic_carbon )
         if (rowarr(ix) > 9) then
           rowarr(ix) = 9
         end if
       end do
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%landscape_type = Area(ix,iy)%temporary
       else
         Area(ix,iy)%landscape_type = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%landscape_type / 1.5 )
         if (rowarr(ix) > 9) then
           rowarr(ix) = 9
         end if
       end do
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%precip_average = Area(ix,iy)%temporary
       else
         Area(ix,iy)%precip_average = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%precip_average / 1000. )
         if (rowarr(ix) > 9) then
           rowarr(ix) = 9
         end if
       end do
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%temperature_average = Area(ix,iy)%temporary
       else
         Area(ix,iy)%temperature_average = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%temperature_average / 70. )
         if (rowarr(ix) > 9) then
           rowarr(ix) = 9
         end if
       end do
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%decid_tree_cover = Area(ix,iy)%temporary
       else
         Area(ix,iy)%decid_tree_cover = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%decid_tree_cover / 10. )
         if (rowarr(ix) > 9) then
           rowarr(ix) = 9
         end if
       end do
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%egreen_tree_cover = Area(ix,iy)%temporary
       else
         Area(ix,iy)%egreen_tree_cover = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%egreen_tree_cover / 10. )
         if (rowarr(ix) > 9) then
           rowarr(ix) = 9
         end if
       end do
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%shrub_cover = Area(ix,iy)%temporary
       else
         Area(ix,iy)%shrub_cover = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%shrub_cover / 10. )
         if (rowarr(ix) > 9) then
           rowarr(ix) = 9
         end if
       end do
   ...
   do iy=1,y_dim
     do ix=1,x_dim
       if (Area(ix,iy)%temporary > 0.) then
         Area(ix,iy)%herb_cover = Area(ix,iy)%temporary
       else
         Area(ix,iy)%herb_cover = 0.
       end if
     end do
   end do
   ! Echo the map
   if (Sim_Parm%echo_maps .eqv. .true.) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         rowarr(ix) = int( Area(ix,iy)%herb_cover / 10. )
         if (rowarr(ix) > 9) then
           rowarr(ix) = 9
         end if
       end do
   ...
   ! *** HERBIVORES ***
   open(SHORT_USE_FILE, FILE=app_path(1:len_trim(app_path))//'\Parms\'//Sim_Parm%herbivore_file_name, ACTION='READ', IOSTAT=ioerr)
   read(SHORT_USE_FILE,*) herbivores
   do i=1, herbivores
     read(SHORT_USE_FILE,*) Herbivore_Parms%spp_name(i)
   end do
   do i=1, herbivores
     read(SHORT_USE_FILE,*) Herbivore_Parms%spp_map(i)
   end do
   ! MUCH MORE HERE TO INCORPORATE HERBIVORE PARAMETERS
   close(SHORT_USE_FILE)

   ! Read in the herbivore maps
   write(*,*) 'Reading in the herbivore population maps.'
   do i=1, herbivores
     call Read_Map ('Herbivores  ', Herbivore_Parms%spp_map(i))
     ! Transfer the map
     do iy=1,y_dim
       do ix=1,x_dim
         if (Area(ix,iy)%temporary > 0.) then
           Area(ix,iy)%herbivore_pop(i) = Area(ix,iy)%temporary
         else
           Area(ix,iy)%herbivore_pop(i) = 0.
         end if
       end do
     end do
   ! Echo the map
   ! Skipping echoing, in so far as herbivore populations will be output, and so may be inspected there.
   end do
...
!***** Read a map into the TEMPORARY variable that is part of the area structure.  That will be transfered
...
!***** R. Boone    Last modified: September 24, 2010
   ...
   if (ioerr == 0) then
     ...
     ! Read the data
     do iy=1,y_dim
       read(SHORT_USE_FILE,*) (Area(ix,iy)%temporary, ix=1,x_dim)
     end do
     close(SHORT_USE_FILE)
   else
...
!*****
!***** R. Boone    Last modified: Feb 10, 2025.    Edited to support use of State Variable file but exclude facet covers
!*****
   use Parameter_Vars
   use Structures
   implicit none
   integer icell, ix, iy, ilayer, ifacet, iunit, plant_count, plant_count2, ispp
   ...
   do icell=1,range_cells
     ix = Rng(icell)%x
     iy = Rng(icell)%y
     Rng(icell)%sand(1) = Area(ix,iy)%top_sand           ! The top and bottom layers get their values directly from the two HWSD layers
     Rng(icell)%sand(4) = Area(ix,iy)%sub_sand           ! The top and bottom layers get their values directly from the two HWSD layers
     Rng(icell)%sand(2) = ( Area(ix,iy)%top_sand * 0.6667 ) + ( Area(ix,iy)%sub_sand * 0.3333 )   ! The other layers get weighted values.
     Rng(icell)%sand(3) = ( Area(ix,iy)%top_sand * 0.3333 ) + ( Area(ix,iy)%sub_sand * 0.6667 )   ! The other layers get weighted values.
      Rng(icell)%silt(1) = Area(ix,iy)%top_silt           ! The top and bottom layers get their values directly from the two HWSD layers
      Rng(icell)%silt(4) = Area(ix,iy)%sub_silt           ! The top and bottom layers get their values directly from the two HWSD layers
      Rng(icell)%silt(2) = ( Area(ix,iy)%top_silt * 0.6667 ) + ( Area(ix,iy)%sub_silt * 0.3333 )   ! The other layers get weighted values.
      Rng(icell)%silt(3) = ( Area(ix,iy)%top_silt * 0.3333 ) + ( Area(ix,iy)%sub_silt * 0.6667 )   ! The other layers get weighted values.
     Rng(icell)%clay(1) = Area(ix,iy)%top_clay           ! The top and bottom layers get their values directly from the two HWSD layers
     Rng(icell)%clay(4) = Area(ix,iy)%sub_clay           ! The top and bottom layers get their values directly from the two HWSD layers
     Rng(icell)%clay(2) = ( Area(ix,iy)%top_clay * 0.6667 ) + ( Area(ix,iy)%sub_clay * 0.3333 )   ! The other layers get weighted values.
     Rng(icell)%clay(3) = ( Area(ix,iy)%top_clay * 0.3333 ) + ( Area(ix,iy)%sub_clay * 0.6667 )   ! The other layers get weighted values.
     ! The following are only used a few lines down, so not storing in Rng, for space-saving purposes.
      gravel(1) = Area(ix,iy)%top_gravel           ! The top and bottom layers get their values directly from the two HWSD layers
      gravel(4) = Area(ix,iy)%sub_gravel           ! The top and bottom layers get their values directly from the two HWSD layers
      gravel(2) = ( Area(ix,iy)%top_gravel * 0.6667 ) + ( Area(ix,iy)%sub_gravel * 0.3333 )   ! The other layers get weighted values.
      gravel(3) = ( Area(ix,iy)%top_gravel * 0.3333 ) + ( Area(ix,iy)%sub_gravel * 0.6667 )   ! The other layers get weighted values.
     bulk_density(1) = Area(ix,iy)%top_bulk_density           ! The top and bottom layers get their values directly from the two HWSD layers
     bulk_density(4) = Area(ix,iy)%sub_bulk_density           ! The top and bottom layers get their values directly from the two HWSD layers
     bulk_density(2) = ( Area(ix,iy)%top_bulk_density * 0.6667 ) + ( Area(ix,iy)%sub_bulk_density * 0.3333 )   ! The other layers get weighted values.
     bulk_density(3) = ( Area(ix,iy)%top_bulk_density * 0.3333 ) + ( Area(ix,iy)%sub_bulk_density * 0.6667 )   ! The other layers get weighted values.
      organic_carbon(1) = Area(ix,iy)%top_organic_carbon           ! The top and bottom layers get their values directly from the two HWSD layers
      organic_carbon(4) = Area(ix,iy)%sub_organic_carbon           ! The top and bottom layers get their values directly from the two HWSD layers
      organic_carbon(2) = ( Area(ix,iy)%top_sand * 0.6667 ) + ( Area(ix,iy)%sub_organic_carbon * 0.3333 )   ! The other layers get weighted values.
      organic_carbon(3) = ( Area(ix,iy)%top_sand * 0.3333 ) + ( Area(ix,iy)%sub_organic_carbon * 0.6667 )   ! The other layers get weighted values.
     ...
     ix = Rng(icell)%x
     iy = Rng(icell)%y
     Rng(icell)%range_type = Area(ix,iy)%landscape_type
     ...
     ! THIS small section will be called more needed, but no matter.
     ! NOTE *****************************************************************
     ! NOTE *** Even though patches in L-Range may be smaller than 1 km2, it
     ! NOTE *** should not matter at all.  The 1000 x 1000 km area will still
     ! NOTE *** represent a theoretical area that can stand in for the grid
     ! NOTE *** grid square, no matter how small.  CHECK THAT POPULATIONS
     ! NOTE *** ARE BEING CALCULATED CORRECTLY.
     ! NOTE *****************************************************************
     do ifacet = 1, FACETS
   ...
   ! forested soils using the grassland initialization is a question.  But we won't be simulating forests, so ...
   do icell=1,range_cells
     ix = Rng(icell)%x
     iy = Rng(icell)%y
     iunit = Rng(icell)%range_type
     temper = Area(ix,iy)%temperature_average
     precip = Area(ix,iy)%precip_average
...
!                                   Rng(icell)%soil_total_carbon * 0.02 + ( ( Rng(icell)%soil_total_carbon * 0.02 ) * 0.011 )
     ...
     ! THIS SECTION NEEDS REVIEWED, MAKE SURE FACETS ARE INITIALIZED
     do ifacet = 1, FACETS
     ...
     ! Then standardize the three surfaces in case they sum to great than 1.0  (they are allowed to be less than 1.0, with the remainder being bare ground)
     temp_sum = Area(Rng(icell)%x, Rng(icell)%y)%herb_cover + Area(Rng(icell)%x, Rng(icell)%y)%shrub_cover + &
                Area(Rng(icell)%x, Rng(icell)%y)%decid_tree_cover + Area(Rng(icell)%x, Rng(icell)%y)%egreen_tree_cover
     if (temp_sum .gt. 100.0) then
       Area(Rng(icell)%x, Rng(icell)%y)%herb_cover = Area(Rng(icell)%x, Rng(icell)%y)%herb_cover * (100.0 / temp_sum )
       Area(Rng(icell)%x, Rng(icell)%y)%shrub_cover = Area(Rng(icell)%x, Rng(icell)%y)%shrub_cover * (100.0 / temp_sum )
       Area(Rng(icell)%x, Rng(icell)%y)%decid_tree_cover = Area(Rng(icell)%x, Rng(icell)%y)%decid_tree_cover * (100.0/temp_sum)
       Area(Rng(icell)%x, Rng(icell)%y)%egreen_tree_cover = Area(Rng(icell)%x, Rng(icell)%y)%egreen_tree_cover * (100.0/temp_sum)
     end if

     ! Facet_cover is the straight proportion of each facet on the 1 km^2.  Facet_population includes understory plants.
     Rng(icell)%facet_cover(T_FACET) = ( Area(Rng(icell)%x, Rng(icell)%y)%decid_tree_cover + &
                                         Area(Rng(icell)%x, Rng(icell)%y)%egreen_tree_cover ) / 100.
     if (Rng(icell)%facet_cover(T_FACET) .gt. 0.0001) then
       Rng(icell)%prop_annual_decid(T_FACET) = Area(Rng(icell)%x, Rng(icell)%y)%decid_tree_cover / &
            ( Area(Rng(icell)%x, Rng(icell)%y)%decid_tree_cover + Area(Rng(icell)%x, Rng(icell)%y)%egreen_tree_cover )
     else
       Rng(icell)%prop_annual_decid(T_FACET) = 0.0
     end if
     ! Shrub cover, which has no good surface to define it (confirmed by Dr. Hansen himself)
     Rng(icell)%facet_cover(S_FACET) = Area(Rng(icell)%x, Rng(icell)%y)%shrub_cover / 100.
     ...
     Rng(icell)%facet_cover(H_FACET) = Area(Rng(icell)%x, Rng(icell)%y)%herb_cover / 100.
     ...
     ! Move the initial herbivore populations to the rangeland cells
     do ispp=1, herbivores
       Rng(icell)%herbivore(ispp) = Area(ix,iy)%herbivore_pop(ispp)
       if (Rng(icell)%herbivore(ispp) .gt. 0) then
         Rng(icell)%herbivore_biomass(ispp) = Herbivore_parms%expected_kg(ispp)
       else
         Rng(icell)%herbivore_biomass(ispp) = 0.0
       end if
     end do
   ...
   if (Sim_Parm%state_var_flag .gt. 1) then
     write(*,*) ' '
   ...
   if (ioerr == 0) then
     ...
     do while (iunit .gt. 0)
       ...
       if (iunit .gt. 0) then
         ...
         read(SHORT_USE_FILE,*) ((Parms(iunit)%tree_carbon(i,j),j=1,2),i=1,WOODY_PARTS)
         write(ECHO_FILE,*) 'Tree carbon, by part, by alive and dead: ',  ((Parms(iunit)%tree_carbon(i,j),j=1,2),i=1,WOODY_PARTS)
         read(SHORT_USE_FILE,*) ((Parms(iunit)%shrub_carbon(i,j),j=1,2),i=1,WOODY_PARTS)
         ...
         read(SHORT_USE_FILE,*) ((Parms(iunit)%temperature_production(i,j),j=1,4),i=1,FACETS)
         write(ECHO_FILE,*) 'Temperature regression points: ',  ((Parms(iunit)%temperature_production(i,j),j=1,4),i=1,FACETS)
...
!         read(SHORT_USE_FILE,*) ((Parms(iunit)%degree_days_phen(i,j),j=1,10),i=1,FACETS)
         ...
         read(SHORT_USE_FILE,*) (Parms(iunit)%herbivore_density(i),i=1,MAX_HERBIVORES)
         write(ECHO_FILE,*) 'Herbivore densities: ',  (Parms(iunit)%herbivore_density(i), i=1,MAX_HERBIVORES)
