...
!**** layers).  Incidentally, a herb that occupies 0.2 x 0.2 m would fill the entire 1 km^2 area with 25,000,000 individuals,
!**** so there is no need for specially defined storage spaces (double and the like)
!****
!****
!****
!**** R. Boone   Last modified: Feb 13, 2013
!****            Feb 13 edit - some death rates converted from annual to monthly - see Plant_Part_Death for fuller description.
!****            January 20, 2025   Edited to include output of coefficients affecting establishment and death rates
  use Parameter_Vars
  use Structures
  implicit none
  ...
  temperature = ( Area(Rng(icell)%x, Rng(icell)%y)%min_temp + Area(Rng(icell)%x, Rng(icell)%y)%max_temp ) / 2.0
  ...
  if (temperature .gt. 0.0) then
     do ifacet = 1, FACETS
       ...
       Rng(icell)%water_effect_on_death_rate(ifacet) = linear(Rng(icell)%ratio_water_pet, data_val, 2)
       do i = 1,4
       ...
       Rng(icell)%grazing_effect_on_death_rate(ifacet) = linear(Rng(icell)%fraction_live_removed_grazing, data_val, 2)
       ...
       select case (ifacet)
         case (H_FACET)
           ...
           do ilayer = H_LYR, H_T_LYR
             ...
             Rng(icell)%shading_effect_on_death_rate(H_FACET) = linear(Rng(icell)%leaf_area_index(H_FACET), data_val, 2)
         ...
         case (S_FACET)
           ...
           do ilayer = S_LYR, S_T_LYR
             ...
             Rng(icell)%shading_effect_on_death_rate(S_FACET) = linear(Rng(icell)%leaf_area_index(S_FACET), data_val, 2)
             death_rate = min(1.0, death_rate)
             death_rate = max(0.0, death_rate)
             died(ilayer) = Rng(icell)%total_population(ilayer) * death_rate
             Rng(icell)%total_population(ilayer) = Rng(icell)%total_population(ilayer) - &
         ...
         case (T_FACET)
           ...
           Rng(icell)%shading_effect_on_death_rate(T_FACET) = linear(Rng(icell)%leaf_area_index(T_FACET), data_val, 2)
           death_rate = min(1.0, death_rate)
           death_rate = max(0.0, death_rate)
           died(ilayer) = Rng(icell)%total_population(ilayer) * death_rate
           Rng(icell)%total_population(ilayer) = Rng(icell)%total_population(ilayer) - &
...
1000 format(A30, I10, I10, I10, F10.7)
  ...
  if (Sim_Parm%fire_maps_used .ne. 0 .or. &
   (Sim_Parm%fire_maps_used .eq. 0 .and. Parms(iunit)%frequency_of_fire .gt. 0.0 &
    ...
    if (Sim_Parm%fire_maps_used .ne. 0) then
      ...
      proportion_cell_burned = Area(Rng(icell)%x, Rng(icell)%y)%prop_burned
    else
  ...
  if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'WHL_DTH')
...
!****
!****
!**** R. Boone   Last modified: January 20, 2025   Edited to include output of coefficients for establishment of plants
!****                                              Note that fine root carbon is at facet level, so herb root effect cannot
!****                                              influence shrub and tree establishment directly.  Has to be herb facet
  use Parameter_Vars
  use Structures
  implicit none
  ...
  if (temperature .gt. 0.0) then
    ...
    do i = 1,4  ;  data_val(i) = Parms(iunit)%water_effect_on_establish(H_FACET,i)  ;  end do
     ...
     Rng(icell)%water_effect_on_establish(H_FACET) = linear(Rng(icell)%ratio_water_pet, data_val, 2)
    do i = 1,4  ;  data_val(i) = Parms(iunit)%water_effect_on_establish(S_FACET,i)  ;  end do
     ...
     Rng(icell)%water_effect_on_establish(S_FACET) = linear(Rng(icell)%ratio_water_pet, data_val, 2)
    do i = 1,4  ;  data_val(i) = Parms(iunit)%water_effect_on_establish(T_FACET,i)  ;  end do
     ...
     Rng(icell)%water_effect_on_establish(T_FACET) = linear(Rng(icell)%ratio_water_pet, data_val, 2)

!    write(ECHO_FILE,'(A40, I5, F9.3, 6(F10.3))') ' REL_EST pre litter FACETS: ',icell, &
!                   Rng(icell)%total_litter_carbon(SURFACE_INDEX), relative_establishment
    ...
    do i = 1,4  ;  data_val(i) = Parms(iunit)%litter_effect_on_establish(H_FACET,i)  ;  end do
     ...
     relative_establishment(H_T_LYR) = relative_establishment(H_T_LYR) * &
                                       linear((Rng(icell)%total_litter_carbon(SURFACE_INDEX) * 2.5), data_val, 2)
     Rng(icell)%litter_effect_on_establish(H_FACET) = linear((Rng(icell)%total_litter_carbon(SURFACE_INDEX) * 2.5), data_val, 2)
    do i = 1,4  ;  data_val(i) = Parms(iunit)%litter_effect_on_establish(S_FACET,i)  ;  end do
     ...
     relative_establishment(S_T_LYR) = relative_establishment(S_T_LYR) * &
                                       linear((Rng(icell)%total_litter_carbon(SURFACE_INDEX) * 2.5), data_val, 2)
     Rng(icell)%litter_effect_on_establish(S_FACET) = linear((Rng(icell)%total_litter_carbon(SURFACE_INDEX) * 2.5), data_val, 2)
    do i = 1,4  ;  data_val(i) = Parms(iunit)%litter_effect_on_establish(T_FACET,i)  ;  end do
     relative_establishment(T_LYR)   = relative_establishment(T_LYR) * &
                                       linear((Rng(icell)%total_litter_carbon(SURFACE_INDEX) * 2.5), data_val, 2)
     Rng(icell)%litter_effect_on_establish(T_FACET) = linear((Rng(icell)%total_litter_carbon(SURFACE_INDEX) * 2.5), data_val, 2)

    ! Do limitation due to herbaceous root biomass limitations.  Only empty patches are candidates for establishment, but that
    ...
    do i = 1,4  ;  data_val(i) = Parms(iunit)%herb_root_effect_on_establish(H_FACET,i)  ;  end do
     ...
     relative_establishment(H_T_LYR) = relative_establishment(H_T_LYR) * &
                                       linear((Rng(icell)%fine_root_carbon(H_FACET) * 2.5), data_val, 2)
     Rng(icell)%herb_root_effect_on_establish(H_FACET) = linear((Rng(icell)%fine_root_carbon(H_FACET) * 2.5), data_val, 2)
    do i = 1,4  ;  data_val(i) = Parms(iunit)%herb_root_effect_on_establish(S_FACET,i)  ;  end do
     relative_establishment(S_LYR)   = relative_establishment(S_LYR) * &
                                       linear((Rng(icell)%fine_root_carbon(H_FACET) * 2.5), data_val, 2)
     relative_establishment(S_T_LYR) = relative_establishment(S_T_LYR) * &
                                       linear((Rng(icell)%fine_root_carbon(H_FACET) * 2.5), data_val, 2)
     Rng(icell)%herb_root_effect_on_establish(S_FACET) = linear((Rng(icell)%fine_root_carbon(H_FACET) * 2.5), data_val, 2)
    do i = 1,4  ;  data_val(i) = Parms(iunit)%herb_root_effect_on_establish(T_FACET,i)  ;  end do
     relative_establishment(T_LYR)   = relative_establishment(T_LYR) * &
                                       linear((Rng(icell)%fine_root_carbon(H_FACET) * 2.5), data_val, 2)
     Rng(icell)%herb_root_effect_on_establish(T_FACET) = linear((Rng(icell)%fine_root_carbon(H_FACET) * 2.5), data_val, 2)
    ...
    do i = 1,4  ;  data_val(i) = Parms(iunit)%woody_cover_effect_on_establish(H_FACET,i)  ;  end do
     ...
     Rng(icell)%woody_cover_effect_on_establish(H_FACET) = linear(Rng(icell)%facet_cover(H_FACET), data_val, 2)
    do i = 1,4  ;  data_val(i) = Parms(iunit)%woody_cover_effect_on_establish(S_FACET,i)  ;  end do
     relative_establishment(S_LYR)   = relative_establishment(S_LYR) * &
                                       linear(Rng(icell)%facet_cover(S_FACET), data_val, 2)
     relative_establishment(S_T_LYR) = relative_establishment(S_T_LYR) * &
                                       linear(Rng(icell)%facet_cover(T_FACET), data_val, 2)
     Rng(icell)%woody_cover_effect_on_establish(S_FACET) = linear(Rng(icell)%facet_cover(S_FACET), data_val, 2)
    do i = 1,4  ;  data_val(i) = Parms(iunit)%woody_cover_effect_on_establish(T_FACET,i)  ;  end do
     relative_establishment(T_LYR)   = relative_establishment(T_LYR) * &
                                       linear(Rng(icell)%facet_cover(T_FACET), data_val, 2)
     Rng(icell)%woody_cover_effect_on_establish(T_FACET) = linear(Rng(icell)%facet_cover(T_FACET), data_val, 2)
...
997 format (A40, I5, 6(I14))
    ...
    if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'PL_REPO')
