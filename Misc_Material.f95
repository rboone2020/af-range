...
!****                           Edited to include CO2 effect on production per year
  use Parameter_Vars
  use Structures
  implicit none
...
!    ! Base N deposition
!    Rng(icell)%base_n_deposition = Parms(iunit)%atmospheric_deposition_parms(1)+ &
!               Parms(iunit)%atmospheric_deposition_parms(2) * min(Area(Rng(icell)%x, Rng(icell)%y)%precip_year, 80.0)
!    if (Rng(icell)%base_n_deposition .lt. 0.) then
    ...
    do ifacet = 1, FACETS
      ...
      Rng(icell)%plant_lignin_fraction(ifacet, SURFACE_INDEX) = Rng(icell)%lignin_leaf(ifacet) + &
                            ...
                            ( Parms(iunit)%lignin_content_fraction_and_precip(2,SURFACE_INDEX) * &
                              Area(Rng(icell)%x, Rng(icell)%y)%precip_average ) / 2.0
...
!****                           Edited to calculate monthly net primary production from total pot prod limited by n by facets
  use Parameter_Vars
  use Structures
  implicit none
...
!! QQ
!if (icell .eq. 5) then
!  write(*,*) 'STC Components POLAR  - Month, FSC, ISC, PSC, LSC, LMC: ', ( ( year - Sim_Parm%start_yr ) * 12 ) + month, &
!     Rng(icell)%fast_soil_carbon(SOIL_INDEX), &
!     Rng(icell)%intermediate_soil_carbon, Rng(icell)%passive_soil_carbon, Rng(icell)%litter_structural_carbon(SOIL_INDEX), &
!     Rng(icell)%litter_metabolic_carbon(SOIL_INDEX)
!end if
!if (icell .eq. 31618) then
!  write(*,*) 'STC Components TROPIC - Month, FSC, ISC, PSC, LSC, LMC: ', ( ( year - Sim_Parm%start_yr ) * 12 ) + month, &
!     Rng(icell)%fast_soil_carbon(SOIL_INDEX), &
!     Rng(icell)%intermediate_soil_carbon, Rng(icell)%passive_soil_carbon, Rng(icell)%litter_structural_carbon(SOIL_INDEX), &
!     Rng(icell)%litter_metabolic_carbon(SOIL_INDEX)
!end if
!! QQ
  ...
  ! ABOVEGROUND
  ! Using method used in productivity.  Does not use plant populations in layers, but uses the facets instead.  Not as precise but less prone to vast swings.
  ...
  if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'UPD_VEG')
...
!**** R. Boone   Last modified: April 24, 2011
  ...
  if (ioerr == 0) then
    ...
    Vals(1) = Exceed%herbivore_population(1)  ;  Vals(2) = Exceed%herbivore_population(2)
    call One_Out ('herbivore_population                    ', 20, Vals)
    Vals(1) = Exceed%herbivore_biomass(1)  ;  Vals(2) = Exceed%herbivore_biomass(2)
    call One_Out ('herbivore_biomass                       ', 17, Vals)
...
!**** R. Boone   Last modified: April 24, 2011
  ...
  ! , total_sec_estimate, total_hr_estimate, elapsed
  ! integer  current_time, cnt_rate, cnt_max, dt(8)
  ...
  ! elapsed = current_time - clock_start_time
  !write(*,*) 'clock_start_time: ', clock_start_time
  !write(*,*) 'current_time: ',current_time, cnt_rate, cnt_max / 1000000.
  !write(*,*) 'elapsed: ',elapsed
  months_to_date = ( ( year - Sim_Parm%start_yr ) * 12 ) + month
  !write(*,*) 'months_to_date: ',months_to_date
  total_months = ( ( Sim_Parm%end_yr - Sim_Parm%start_yr ) + 1 ) * 12
  !write(*,*) 'total_months: ',total_months
  progress_to_date = ( months_to_date / total_months ) * 100.0
  !write(*,*) 'progress_to_date: ',progress_to_date
  !total_sec_estimate = ( total_months / progress_to_date ) * elapsed
  !write(*,*) 'total_sec_estimate: ',total_sec_estimate
  !total_hr_estimate = ( total_sec_estimate / 60.0 ) / 60.0
  !write(*,*) 'total_hr_estimate: ',total_hr_estimate

  !write(*,"(A6,I4,A8,I2,A22,F5.1,A30,F8.2,A15)") 'Year: ', year, ' Month: ', month, '   Progress: ', progress_to_date, &
  !           '%, about ', total_hr_estimate, ' hour(s) until done.'
...
!****                           Fraction of grazed material changed to capture Af_Range perspective and individual-cell differences.
  use Parameter_Vars
  use Structures
  implicit none

  real    live_carbon, dead_carbon, live_carbon_removed, dead_carbon_removed
...
!  end if
  ...
  !  herbivore
  do i=1,herbivores
    if (Rng(icell)%herbivore(i) .gt. V_LARGE) then
      Rng(icell)%herbivore(i) = V_LARGE
      Rng(icell)%large_error_count = Rng(icell)%large_error_count + 1
      Exceed%herbivore_population(1) = Exceed%herbivore_population(1) + 1
    end if
    if (Rng(icell)%herbivore(i) .lt.   0.0  ) then
      Rng(icell)%herbivore(i) = 0.0
      Rng(icell)%neg_error_count = Rng(icell)%neg_error_count + 1
      Exceed%herbivore_population(2) = Exceed%herbivore_population(2) + 1
    end if
  end do

  !  herbivore_biomass
  do i=1,herbivores
    if (Rng(icell)%herbivore_biomass(i) .gt. V_LARGE) then
      Rng(icell)%herbivore_biomass(i) = V_LARGE
      Rng(icell)%large_error_count = Rng(icell)%large_error_count + 1
      Exceed%herbivore_biomass(1) = Exceed%herbivore_biomass(1) + 1
    end if
    if (Rng(icell)%herbivore_biomass(i) .lt.   0.0  ) then
      Rng(icell)%herbivore_biomass(i) = 0.0
      Rng(icell)%neg_error_count = Rng(icell)%neg_error_count + 1
      Exceed%herbivore_biomass(2) = Exceed%herbivore_biomass(2) + 1
    end if
  end do
  ...
  ! Note timing sensitivity.  This summary is happening after grazing, and so reports based on remaining biomass. Must it be done before grazing?
  live_carbon = 0.0
  dead_carbon = 0.0
  live_carbon_removed = 0.0
  dead_carbon_removed = 0.0

  ! Recalculating live carbon for security, to ensure fair comparison
  live_carbon = Rng(icell)%leaf_carbon(H_FACET)
  live_carbon_removed = Rng(icell)%grazed_green_herb_fraction * Rng(icell)%leaf_carbon(H_FACET)
  live_carbon = live_carbon + Rng(icell)%seed_carbon(H_FACET)
  live_carbon_removed = live_carbon_removed + (Rng(icell)%grazed_green_herb_fraction * Rng(icell)%seed_carbon(H_FACET))
  dead_carbon = Rng(icell)%dead_standing_carbon(H_FACET)
  dead_carbon_removed = Rng(icell)%grazed_dead_herb_fraction * Rng(icell)%dead_standing_carbon(H_FACET)

  live_carbon = live_carbon + Rng(icell)%leaf_carbon(S_FACET)
  live_carbon_removed = live_carbon_removed + Rng(icell)%grazed_green_shrub_fraction * Rng(icell)%leaf_carbon(S_FACET)
  live_carbon = live_carbon + Rng(icell)%seed_carbon(S_FACET)
  live_carbon_removed = live_carbon_removed + (Rng(icell)%grazed_green_shrub_fraction * Rng(icell)%seed_carbon(S_FACET))
  dead_carbon = dead_carbon + Rng(icell)%dead_standing_carbon(S_FACET)
  dead_carbon_removed = dead_carbon_removed + Rng(icell)%grazed_dead_shrub_fraction * Rng(icell)%dead_standing_carbon(S_FACET)
  live_carbon = live_carbon + Rng(icell)%fine_branch_carbon(S_FACET)
  live_carbon_removed = live_carbon_removed + (Rng(icell)%grazed_fbranch_shrub_fraction * Rng(icell)%fine_branch_carbon(S_FACET))

  live_carbon = live_carbon + Rng(icell)%leaf_carbon(T_FACET)
  live_carbon_removed = live_carbon_removed + Rng(icell)%grazed_green_shrub_fraction * Rng(icell)%leaf_carbon(T_FACET)
  live_carbon = live_carbon + Rng(icell)%seed_carbon(T_FACET)
  live_carbon_removed = live_carbon_removed + (Rng(icell)%grazed_green_shrub_fraction * Rng(icell)%seed_carbon(T_FACET))
  dead_carbon = dead_carbon + Rng(icell)%dead_standing_carbon(T_FACET)
  dead_carbon_removed = dead_carbon_removed + Rng(icell)%grazed_dead_shrub_fraction * Rng(icell)%dead_standing_carbon(T_FACET)
  live_carbon = live_carbon + Rng(icell)%fine_branch_carbon(S_FACET)
  live_carbon_removed = live_carbon_removed + (Rng(icell)%grazed_fbranch_shrub_fraction * Rng(icell)%fine_branch_carbon(S_FACET))

  if (live_carbon .gt. 0.00000001) then
    Rng(icell)%fraction_live_removed_grazing = live_carbon_removed / live_carbon
  else
    Rng(icell)%fraction_live_removed_grazing = 0.0
  end if

  if (dead_carbon .gt. 0.00000001) then
    Rng(icell)%fraction_dead_removed_grazing = dead_carbon_removed / dead_carbon
  else
    Rng(icell)%fraction_dead_removed_grazing = 0.0
  end if
 ...
 if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'EACH_MN')
...
!**** R. Boone    Last modified: May 31, 2011
 ...
 if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'ZERO_AC')
...
!**** R. Boone    Last modified: May 31, 2011
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         if (Area(ix,iy)%temporary .lt. 0) then
           Area(ix,iy)%prop_burned = 0
         else
           Area(ix,iy)%prop_burned = Area(ix,iy)%temporary
         end if
       end do
     end do
   ...
   if (Sim_Parm%fertilize_maps_used .ne. 0) then
     ...
     do iy=1,y_dim
       do ix=1,x_dim
         if (Area(ix,iy)%temporary .lt. 0) then
           Area(ix,iy)%prop_fertilized = 0
         else
           Area(ix,iy)%prop_fertilized = Area(ix,iy)%temporary
         end if
       end do
     end do
     ! These maps are not echoed, due to their large size.  Specific maps may be printed as output, rather than echoed, as a check of their form.
...
!**** R. Boone    Last modified: May 31, 2011
  ...
  if (Sim_Parm%fertilize_maps_used .eq. 1 .or. &
   (Sim_Parm%fertilize_maps_used .eq. 0 .and. Parms(iunit)%frequency_of_fertilization .gt. 0.0 &
     .and. Parms(iunit)%fraction_fertilized .gt. 0.0)) then
    if (Sim_Parm%fertilize_maps_used .eq. 1 ) then
      ...
      proportion_cell_fertilized = Area(Rng(icell)%x, Rng(icell)%y)%prop_fertilized
    else
...
! else ... no fertilization
  ...
  if (check_nan_flag .eqv. .TRUE.) call check_for_nan (icell, 'MANAGED')
...
!**** R. Boone    Last modified: July 3, 2014
  ...
  integer  icell, ifacet, ilyr, ipart, ispp
  character*7  calling
  ...
  if ( Rng(icell)%x .ne. Rng(icell)%x ) then
    ...
    if ( stop_on_nan_flag .eqv. .TRUE.) then
      stop
    else
  ...
  if ( Rng(icell)%day_length_increasing .neqv. Rng(icell)%day_length_increasing ) then
...
!  end do
  ...
  do ispp=1,herbivores
   if ( Rng(icell)%herbivore(ispp) .ne. Rng(icell)%herbivore(ispp) ) then
    write(*,*) 'A -NaN error has occurred in rangeland variable herbivore biomass for species ', ispp, ' ', icell, ' called by ', &
               calling
    if ( stop_on_nan_flag .eqv. .TRUE.) then
      stop
    else
      write(*,*) 'Warning, resetting to zero for cell ', icell,' for variable herbivore ', ispp, ' due to -NaN.'
      Rng(icell)%herbivore(ispp) = 0.0
    end if
   end if
  end do
  ...
  ! Herbivore_biomass
  do ispp=1,herbivores
   if ( Rng(icell)%herbivore_biomass(ispp) .ne. Rng(icell)%herbivore_biomass(ispp) ) then
    write(*,*) 'A -NaN error has occurred in rangeland variable herbivore biomass for species ', ispp, ' ', icell, ' called by ', &
               calling
    if ( stop_on_nan_flag .eqv. .TRUE.) then
      stop
    else
      write(*,*) 'Warning, resetting to zero for cell ', icell,' for herbivore biomass ', ispp, ' due to -NaN.'
